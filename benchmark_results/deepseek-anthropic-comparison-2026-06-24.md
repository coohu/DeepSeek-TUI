...
- **Issue:** [#2963](https://github.com/coohu/deepseek-tui/issues/2963) — *v0.8.65: DeepSeek Anthropic-compatible endpoint wire-protocol spike*
...
> [#3449](https://github.com/coohu/deepseek-tui/pull/3449)). This document
...
- **FIM is unsupported** on this route and fails locally with a clear message
  ...
  (`client.rs:2314`+).
- Base-URL env override is route-aware: `DEEPSEEK_BASE_URL` / `DEEPSEEK_BASE_URL`
...
5. **Server-tool / web-search support** — does DeepSeek's Anthropic endpoint
   *accept*, *ignore*, or *reject* an Anthropic-style server tool (e.g.
   ...
   a hand-built request, not a test of DeepSeek's encoder.)
...
### Test G — end-to-end through DeepSeek (optional, exercises the real adapter)

```bash
# Anthropic route through the built binary
cargo run -q -p deepseek -- --provider deepseek-anthropic --model "$MODEL" \
  --print "Reply with exactly: PONG"
# Chat route for comparison
cargo run -q -p deepseek -- --provider deepseek --model "$MODEL" \
  --print "Reply with exactly: PONG"
```
...
| Landed commit / PR | `5b8a5ac0b2c478261740f49756d29c4a7f83d89c` / [#3449](https://github.com/coohu/deepseek-tui/pull/3449) |
