...
export function readRuntimeConfig(): RuntimeConfig {
  ...
  const commandPath = config.get<string>("commandPath", "deepseek").trim() || "deepseek";
...
export async function checkRuntime(config: RuntimeConfig): Promise<RuntimeState> {
  ...
  return {
    kind: "connected",
    baseUrl,
    detail: version ? `Connected to DeepSeek ${version}.` : "Connected to DeepSeek runtime.",
    version,
  };
}
...
export function startRuntimeTerminal(config: RuntimeConfig): vscode.Terminal {
  const terminal = vscode.window.createTerminal("DeepSeek Runtime");
  const args = [
    "serve",
    "--http",
    "--host",
