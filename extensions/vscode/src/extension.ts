...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("deepseek.openTerminal", () => {
      const config = readRuntimeConfig();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("deepseek.startRuntime", () => {
      const config = readRuntimeConfig();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("deepseek.checkRuntime", async () => {
      return await checkAndRefreshRuntime(true, true);
    }),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("deepseek.refreshAgentView", async () => {
      await refreshAgentViewDetails(true);
    }),
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("deepseek.refreshSnapshots", async () => {
      try {
        await refreshSnapshots();
      } catch (error: unknown) {
        const detail = error instanceof Error ? error.message : String(error);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("deepseek.openRuntimeDocs", () => {
      void vscode.env.openExternal(
        vscode.Uri.parse(
          "https://github.com/coohu/deepseek-tui/blob/main/docs/RUNTIME_API.md",
        ),
      );
    }),
  );
