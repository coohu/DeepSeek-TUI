...
Official DeepSeek extension scaffold for local development.

This first slice is intentionally small:

- open DeepSeek in an integrated terminal
- start `deepseek serve --http` in a visible terminal
...
npm run compile
npm run package
code --install-extension deepseek-vscode-0.8.53.vsix
```

Configure `deepseek.commandPath`, `deepseek.runtimeHost`,
`deepseek.runtimePort`, `deepseek.runtimeToken`, and
`deepseek.agentViewRefreshIntervalSeconds` from VS Code settings.
