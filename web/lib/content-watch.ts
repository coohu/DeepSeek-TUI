...
const LINK_TARGETS: { url: string; label: string }[] = [
  { url: "https://github.com/coohu/", label: "Main repo" },
  { url: "https://github.com/coohu//issues", label: "Issues" },
  { url: "https://github.com/coohu//pulls", label: "Pull Requests" },
  { url: "https://github.com/coohu//discussions", label: "Discussions" },
  { url: "https://github.com/coohu//releases", label: "Releases" },
  { url: "https://github.com/coohu//blob/main/LICENSE", label: "License file" },
  { url: "https://github.com/coohu//blob/main/CODE_OF_CONDUCT.md", label: "Code of Conduct" },
  { url: "https://github.com/coohu//blob/main/SECURITY.md", label: "Security policy" },
  { url: "https://github.com/coohu//blob/main/CONTRIBUTING.md", label: "Contributing guide" },
...
export async function runSemanticDrift(env: WatchEnv): Promise<{ ok: boolean; drafted: number; reason?: string }> {
  ...
  const [changelog, commits, homepageHtml, docsHtml] = await Promise.all([
    fetch("https://raw.githubusercontent.com/coohu//main/CHANGELOG.md", { headers: ghHeaders }).then((r) => r.ok ? r.text() : "").catch(() => ""),
    fetch("https://api.github.com/repos/coohu//commits?per_page=30", { headers: ghHeaders }).then((r) => r.ok ? r.json() as Promise<{ commit: { message: string }; sha: string }[]> : []).catch(() => []),
    fetch("https://.com/en", { headers: { "User-Agent": "-watch" } }).then((r) => r.ok ? r.text() : "").catch(() => ""),
    fetch("https://.com/en/docs", { headers: { "User-Agent": "-watch" } }).then((r) => r.ok ? r.text() : "").catch(() => ""),
  ]);
