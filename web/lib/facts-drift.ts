...
const RAW_BASE = "https://raw.githubusercontent.com/coohu/deepseek-tui/main";
...
async function fetchListing(dir: string, ghToken?: string): Promise<string[] | null> {
  ...
  const url = `https://api.github.com/repos/coohu/deepseek-tui/contents/${dir}?ref=main`;
  const headers: Record<string, string> = {
...
async function fetchLatestRelease(ghToken?: string): Promise<string | null> {
  ...
  if (ghToken) headers["Authorization"] = `Bearer ${ghToken}`;
  try {
    const r = await fetch("https://api.github.com/repos/coohu/deepseek-tui/releases/latest", { headers });
    if (!r.ok) return null;
