...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: isZh ? "嵌套宪法 · DeepSeek 文档" : "Constitution · DeepSeek Docs",
    description: isZh
      ...
      <section id="overview" className="scroll-mt-32">
        ...
        {isZh ? (
          <p className="text-ink-soft mt-3 leading-[1.9] tracking-wide">
            DeepSeek 先给 Agent 一个可追责的地址，再给上下文冲突一套法律。全局 Constitution 处理
            truth、user agency、行动和验证；仓库可以通过{" "}
            <code className="inline">.deepseek/constitution.json</code> 增加本地 law；runtime
            policy 再把模式、审批、沙箱、成本和工具边界落到代码里。
          </p>
        ) : (
          <p className="text-ink-soft mt-3 leading-relaxed">
            DeepSeek gives the agent an accountable address, then a legal system for
            context conflicts. The global Constitution handles truth, user agency,
            action, and verification; repos can add local law via{" "}
            <code className="inline">.deepseek/constitution.json</code>; runtime policy
        ...
        <p className={`mt-4 text-sm text-ink-soft ${isZh ? "leading-[1.9] tracking-wide" : "leading-relaxed"}`}>
          {isZh
            ? "普通项目说明仍放在 AGENTS.md；DeepSeek 专属的冲突解决和验证策略放在 .deepseek/constitution.json。详见 "
            : "Standard project instructions still live in AGENTS.md; DeepSeek-specific conflict resolution and verification policies go in .deepseek/constitution.json. See "}
          <Link
            href="https://github.com/coohu/deepseek-tui/blob/main/docs/CONFIGURATION.md#project-instructions--repo-authority"
            className="body-link"
          >
