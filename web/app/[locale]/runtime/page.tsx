...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildPageMetadata({
    path: "/runtime",
    locale,
    title: isZh ? "Runtime & 集成 · DeepSeek" : "Runtime & Integrations · DeepSeek",
    description: isZh
      ? "DeepSeek 的本地 Runtime API、HTTP/SSE、ACP 协议、MCP 服务器、VS Code 扩展、Telegram / Feishu 桥接以及实验性微信集成。"
      : "DeepSeek local Runtime API, HTTP/SSE, ACP protocol, MCP servers, VS Code extension, Telegram and Feishu bridges, and experimental Weixin integration.",
  });
}
...
const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    desc: "Open IETF-standard protocol surface for agent-to-agent communication. DeepSeek speaks ACP natively so external agents, tools, and platforms can discover and interact with running sessions.",
    descZh: "开放的 IETF 标准 Agent 通信协议。DeepSeek 原生支持 ACP，外部 Agent、工具和平台可以发现并互操作运行中的会话。",
    docsHref: "/en/docs#acp",
  },
  {
    name: "MCP (Model Context Protocol)",
    desc: "Connect DeepSeek to external tools and services via MCP servers over stdio or HTTP/SSE. Pre-configured servers include filesystem, Git, SQLite, and popular SaaS platforms.",
    descZh: "通过 MCP 服务器（stdio 或 HTTP/SSE）将 DeepSeek 连接到外部工具和服务。预配置的服务器包括文件系统、Git、SQLite 和常用 SaaS 平台。",
    docsHref: "/en/docs#mcp",
  },
  {
    name: "VS Code Extension",
    desc: "Open-source VS Code extension that embeds DeepSeek as a side-panel agent. Run deepseek inside your editor with full workspace context.",
    descZh: "开源 VS Code 扩展，将 DeepSeek 嵌入编辑器的侧边面板。在编辑器内利用完整工作区上下文运行 DeepSeek。",
    href: "https://github.com/coohu/deepseek-tui/tree/main/extensions/vscode",
  },
  {
    name: "Telegram Bridge",
    desc: "First-party Telegram bot bridge. Start a headless DeepSeek session, then chat with it from any Telegram client — approvals, tool results, and completions surface inline.",
    descZh: "官方 Telegram 机器人桥接。启动无头 DeepSeek 会话，在任何 Telegram 客户端中与之对话——审批、工具结果和完成状态内联展示。",
    href: "https://github.com/coohu/deepseek-tui/tree/main/integrations/telegram-bridge",
  },
  {
    ...
    href: "https://github.com/coohu/deepseek-tui/tree/main/integrations/feishu-bridge",
  },
  {
    ...
    href: "https://github.com/coohu/deepseek-tui/tree/main/integrations/weixin-bridge",
  },
];
    ...
    <>
      ...
      <section className="mx-auto max-w-[1100px] px-6 pt-12 pb-10">
        ...
        <p className="max-w-2xl text-ink-soft leading-relaxed">
          {isZh
            ? "DeepSeek 不仅是一个终端 Agent——它还是一个可通过多种协议和集成方式嵌入到你现有工作流中的本地控制平面。"
            : "DeepSeek is more than a terminal agent — it is a local control plane you can embed into your existing workflow through multiple protocols and integrations."}
        </p>
      </section>
      ...
      <section className="mx-auto max-w-[1100px] px-6 py-8 hairline-t">
        ...
        <div className="grid sm:grid-cols-2 gap-6 text-sm text-ink-soft leading-relaxed">
          ...
          <div>
            ...
            <p className="mt-1">
              {isZh
                ? "所有 Runtime API 路由（/v1/*）需要 Bearer Token。配置 DEEPSEEK_RUNTIME_TOKEN 环境变量或 config.toml 中的 auth_token。"
                : "All Runtime API routes (/v1/*) require a Bearer token. Set DEEPSEEK_RUNTIME_TOKEN env var or auth_token in config.toml."}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-1">
              {isZh
                ? "DeepSeek 使用标准 HTTP/SSE、JSON-RPC 和 ACP，可与任何兼容客户端或平台集成。"
                : "DeepSeek speaks standard HTTP/SSE, JSON-RPC, and ACP — compatible with any client or platform."}
            </p>
          </div>
        </div>
      </section>
