...
3. **Point the domain:** in the Cloudflare dashboard, add a Worker route for `deepseek.net/*` → the deployed Worker, named `deepseek-web` (see `wrangler.jsonc`).
...
1. **Build time** — `scripts/derive-facts.mjs` runs as `prebuild` (and before
   ...
   `crates/tui/src/sandbox/`, `npm/deepseek/package.json`) and writes
