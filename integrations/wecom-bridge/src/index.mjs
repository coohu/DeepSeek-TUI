...
const config = {
  ...
  threadMapPath: process.env.WECOM_THREAD_MAP_PATH || "/var/lib/deepseek-wecom-bridge/thread-map.json",
  ...
  approvalTimeoutMs: Number(process.env.DEEPSEEK_APPROVAL_TIMEOUT_MS || 300000)
};
...
console.log("Starting DeepSeek WeCom bridge");
...
async function handleEvent(frame) {
  ...
  if (eventType === "enter_chat") {
    ...
    if (chatId) {
      await client.replyWelcome(frame, { msgtype: "text", text: { content: "欢迎使用 DeepSeek！发送 /help 查看可用命令。" } });
    }
  }
}
