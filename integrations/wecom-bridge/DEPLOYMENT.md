...
The WeCom Bridge integrates DeepSeek with WeCom (企业微信) Smart Bot
...
2. **DeepSeek runtime API** running at `http://127.0.0.1:7878`
...
```bash
export DEEPSEEK_RUNTIME_TOKEN="$(openssl rand -hex 32)"
deepseek serve --http --host 127.0.0.1 --port 7878 --auth-token "$DEEPSEEK_RUNTIME_TOKEN"
```
...
cp .env.example .env
# Edit .env with your WeCom credentials and the same DEEPSEEK_RUNTIME_TOKEN.
npm install
npm run start
```
...
| `DEEPSEEK_RUNTIME_TOKEN` | `rand-xxxxxxxx` | Bearer token for Runtime API (generate a random string) |

### Optional variables

| Variable | Default | Description |
|----------|---------|-------------|
| `DEEPSEEK_RUNTIME_URL` | `http://127.0.0.1:7878` | Runtime API address |
| `DEEPSEEK_WORKSPACE` | `(cwd)` | Workspace directory |
| `DEEPSEEK_MODEL` | `auto` | Default model name |
...
| `DEEPSEEK_TURN_TIMEOUT_MS` | `900000` | Turn timeout in ms (15 min) |
| `DEEPSEEK_APPROVAL_TIMEOUT_MS` | `300000` | Approval timeout in ms (5 min) |
...
```
WeCom Client → Smart Bot WebSocket → WeCom Bridge ──HTTP──→ deepseek serve --http
...
- **No public port exposure**: `deepseek serve --http` binds to `127.0.0.1` only
- **Token authentication**: all `/v1/*` runtime calls require `DEEPSEEK_RUNTIME_TOKEN`
...
| 404 on `/allow` | Approval ID expired (5 min) | Respond faster, or increase `DEEPSEEK_APPROVAL_TIMEOUT_MS` |
...
- [DeepSeek Security Policy](../../SECURITY.md)
- [DeepSeek Contributing Guide](../../CONTRIBUTING.md)
