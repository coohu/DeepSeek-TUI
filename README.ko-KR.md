...
[English README](README.md) · [简体中文 README](README.zh-CN.md) · [日本語 README](README.ja-JP.md) · [Tiếng Việt README](README.vi.md) · [deepseek.net](https://deepseek.net/) · [Install guide](docs/INSTALL.md) · [Provider registry](docs/PROVIDERS.md) · [Changelog](CHANGELOG.md)
...
![터미널에서 실행 중인 DeepSeek](assets/screenshot.png)
...
다운로드하고 `deepseek`, `codew`, `deepseek-tui`를 설치합니다.
...
[GitHub Releases](https://github.com/coohu/deepseek-tui/releases)에
...
확인한 다음 `deepseek doctor`를 실행해 확인하세요.
...
대신 Claude 키가 있다면 `deepseek auth set --provider anthropic`을
실행하거나 `ANTHROPIC_API_KEY`만 내보내세요. 이후에는 네이티브 Messages
어댑터가 처리합니다.

키는 `~/.deepseek/config.toml`에 저장됩니다. 레거시 `~/.deepseek/`
설정도 호환성을 위해 계속 읽습니다.
...
- `/fleet`은 Fleet 설정 뷰를 엽니다 — 역할, 프로필, 로드아웃, 정책을
  다룹니다.
- `/skills`는 `~/.deepseek/skills/`에서 재사용 가능한 워크플로를
  불러옵니다.
- `/config`는 런타임 설정을 편집하고, `/statusline`은 라우트, 비용,
  세션 상태를 표시할 푸터 칩을 고릅니다.
- `! cargo test -p deepseek-tui`는 일반적인 승인 및 샌드박스 경로를
  통해 임의의 셸 명령을 실행합니다.
...
fleet 워커는 헤드리스 `deepseek exec` 실행이지만, Fleet은 이를
내구성 있게 시작하고 추적합니다. 작업은 추가 전용 원장
(`.deepseek/fleet.jsonl`)에 기록되므로 매니저 종료, 노트북 절전,
...
- **승인 게이트가 있는 도구.** `.deepseek/hooks.toml` 훅 시스템은 모든
...
- **헤드리스 모드.** 스크립트와 CI를 위해 `--allowed-tools`,
  ...
  `--append-system-prompt`를 사용하는 `deepseek exec`.
- **양방향 MCP.** 외부 MCP 서버의 도구를 사용하거나, `deepseek mcp`를
  통해 DeepSeek 자체를 MCP 서버로 노출할 수 있습니다.
- **스킬.** `/skills`로 불러오는 `~/.deepseek/skills/`의 재사용 가능한
  워크플로입니다.
...
2. **프로젝트의 법** — 저장소에 `.deepseek/constitution.json`을 두어
...
[deepseek.net](https://deepseek.net/)에 있습니다:
...
- [열려 있는 이슈](https://github.com/coohu/deepseek-tui/issues) — 처음
...
[![Star History Chart](https://api.star-history.com/chart?repos=coohu/deepseek-tui&type=date&legend=top-left)](https://www.star-history.com/?repos=Hmbown%2FCodeWhale&type=date&logscale=&legend=top-left)
