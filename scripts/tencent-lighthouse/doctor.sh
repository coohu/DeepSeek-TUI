...
case "${BRIDGE_KIND}" in
  feishu|lark)
    if [[ -z "${BRIDGE_ENV:-}" ]]; then
      if [[ -f /etc/deepseek/feishu-bridge.env || ! -f /etc/deepseek/feishu-bridge.env ]]; then
  ...
  telegram)
    if [[ -z "${BRIDGE_ENV:-}" ]]; then
      if [[ -f /etc/deepseek/telegram-bridge.env || ! -f /etc/deepseek/telegram-bridge.env ]]; then
...
=======
  for unit in deepseek-runtime "${BRIDGE_UNIT}"; do
>>>>>>> 9463266cb1278cdb82ada338479f32d47f31a704
...
main() {
  printf 'Tencent Lighthouse DeepSeek doctor (%s bridge)\n' "${BRIDGE_KIND}"
  check_commands
  check_node
  check_workspace
  check_binaries
