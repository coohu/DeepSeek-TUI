...
# Launches the compiled deepseek-tui binary on loopback ports and verifies
# the mobile control page, auth, API routes, and binding behaviour through
# real HTTP requests.
#
# Usage:  ./scripts/mobile-smoke.sh
# Requires: curl, a built binary at target/release/deepseek-tui
...
BINARY="${BINARY:-${REPO_ROOT}/target/release/deepseek-tui}"
PASS=0
FAIL=0
SERVER_PID=""
...
if [[ ! -x "$BINARY" ]]; then
    log "Binary not found; building deepseek-tui in release mode..."
    cargo build -p deepseek-tui --release --locked
fi
...
assert_body_contains GET "/mobile?token=${TOKEN}" "" "DeepSeek Mobile"
...
assert_body_contains GET "/mobile" "" "DeepSeek Mobile"
