...
This concretizes `docs/REMOTE_VM_US.md`: a cheap US VPS running the DeepSeek
...
- `agent-session.sh` — sourceable helper for interactive/tmux agent sessions
  as the `deepseek` user.  Sources `/etc/deepseek/runtime.env` so the
...
- Secrets travel as a chmod-600 file over scp, land in `/etc/deepseek/*.env`
  (0640 root:deepseek), and the transfer file is shredded. Never in argv,
...
- The Rust binary reads only `DEEPSEEK_RUNTIME_TOKEN`/`--auth-token` and
  `--port`; the `DEEPSEEK_RUNTIME_*` names in `/etc/deepseek/runtime.env`
  work because the systemd unit expands them into flags. Don't start
  `deepseek serve` by hand and expect the env file to apply.
- `deepseek-runtime.service` hard-fails activation if
  `/home/deepseek/.deepseek` or `/home/deepseek/.deepseek` don't exist
  (`ReadWritePaths`); `setup-vm.sh` pre-creates them.
- Both binaries are required (`deepseek` delegates to `deepseek-tui`).
...
fine-grained PAT (scoped to coohu/deepseek-tui: Contents RW, Issues RW,
...
gh issue list --repo coohu/deepseek-tui --milestone v0.8.58 \
...
git -C /opt/whalebro/deepseek fetch origin
git -C /opt/whalebro/deepseek worktree add \
...
gh issue view <N> --json body -q .body | \
  deepseek exec --auto --output-format stream-json "$(cat)"

# 5. Verify (run the issue's Verification block verbatim)
# 6. Deliver
gh pr create --repo coohu/deepseek-tui --base main \
