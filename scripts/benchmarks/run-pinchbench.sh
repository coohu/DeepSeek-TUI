...
# API key from ~/.deepseek/config.toml if not set via environment variables.
#
# Usage:
...
#   - Python 3.10+ with uv
#   - Xiaomi MiMo API key (in env or ~/.deepseek/config.toml)
...
CODEWHALE_CONFIG="${HOME}/.deepseek/config.toml"
...
Environment variables (direct mode):
  XIAOMI_MIMO_API_KEY     Xiaomi MiMo API key (or XIAOMI_API_KEY / MIMO_API_KEY)
                          Falls back to ~/.deepseek/config.toml if unset
...
# ── Read MiMo config from ~/.deepseek/config.toml ──────────────────────────
# Extracts api_key and base_url from [providers.xiaomi_mimo] section.
read_deepseek_mimo_config() {
    local config="$1"
    local key="" url=""
    if [[ -f "$config" ]]; then
...
# ── Direct MiMo mode (default) ─────────────────────────────────────────────
    ...
    # Resolve API key: env var > deepseek config.toml
    MIMO_KEY="${XIAOMI_MIMO_API_KEY:-${XIAOMI_API_KEY:-${MIMO_API_KEY:-}}}"

    if [[ -z "$MIMO_KEY" ]]; then
        # Try reading from deepseek config
        IFS='|' read -r cfg_key cfg_url <<< "$(read_deepseek_mimo_config "$CODEWHALE_CONFIG")"
        if [[ -n "$cfg_key" ]]; then
            MIMO_KEY="$cfg_key"
...
{
    "deepseek_version": "$(deepseek --version 2>/dev/null || echo unknown)",
