#!/usr/bin/env python3
"""
DeepSeek-native PinchBench runner.

Loads PinchBench tasks, runs them through deepseek exec, and grades results.
...
def prepare_workspace(task: dict, run_dir: Path, tasks_dir: Path) -> Path:
    ...
    # Initialize git repo so deepseek works
    subprocess.run(["git", "init"], cwd=workspace, capture_output=True, check=False)
    subprocess.run(
        ["git", "config", "user.email", "bench@deepseek"],
...
def run_codewhale(
    ...
    """Run deepseek exec on a task and return the result."""
    cmd = [
        "deepseek", "exec",
        "--auto",
...
def grade_llm_judge(task: dict, workspace: Path, transcript: list, model: Optional[str] = None) -> dict[str, Any]:
    """Use deepseek as an LLM judge to grade a task."""
...
Be strict but fair. Partial credit is OK."""
    ...
    cmd = ["deepseek", "exec", "--auto", "--workspace", str(workspace)]
    if model:
        cmd.extend(["--model", model])
...
def run_benchmark(
    ...
    cw_version = "unknown"
    try:
        vr = subprocess.run(["deepseek", "--version"], capture_output=True, text=True)
        if vr.returncode == 0:
    ...
    for i, task in enumerate(all_tasks, 1):
        ...
        # Run deepseek
        print(f"  Running deepseek exec (timeout: {timeout}s)...")
    ...
    print(f"\n{'='*60}")
    print(f"PINCHBENCH SCORE SUMMARY (DeepSeek)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run PinchBench tasks through DeepSeek (no OpenClaw)"
    )
    parser.add_argument(
        "--tasks-dir",
        type=Path,
    ...
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("./results/pinchbench-deepseek"),
        help="Results output directory",
    )
    parser.add_argument("--model", default=None, help="Model override for deepseek")
    parser.add_argument(
        "--timeout-multiplier",
        type=float,
        default=1.0,
