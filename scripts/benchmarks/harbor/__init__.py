"""
Harbor adapter for DeepSeek.

Lets Harbor evaluate DeepSeek as an agent on Terminal-Bench and other
...
Usage (after pip install harbor):
    ...
    harbor run \\
      ...
      --agent scripts.benchmarks.harbor.deepseek_agent:CodeWhaleAgent \\
...
class CodeWhaleAgent(BaseInstalledAgent):
    """
    DeepSeek agent adapter for Harbor.

    Installs the ``deepseek`` CLI via npm into the task container and runs
    tasks in non-interactive exec mode with full tool access.
    """

    _OUTPUT_FILENAME = "deepseek.txt"
    ...
    def get_version_command(self) -> str | None:
        return "deepseek --version 2>/dev/null || deepseek-tui --version 2>/dev/null"
    ...
    def parse_version(self, stdout: str) -> str:
        ...
        for line in text.splitlines():
            line = line.strip()
            if line:
                # Strip any prefix like "deepseek " or "deepseek-cli "
                for prefix in ("deepseek-tui ", "deepseek-cli ", "deepseek "):
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        """Install DeepSeek via npm in the container."""
        ...
        # Install DeepSeek CLI via npm
        await self.exec_as_agent(
            environment,
            command="npm install -g deepseek",
        )
    ...
    async def run(
        ...
        """Run DeepSeek in non-interactive exec mode on the task."""
        ...
        model_flag = ""
        if self.model_name:
            # Harbor passes model as "provider/model"; DeepSeek uses --model
            model_flag = f"--model {shlex.quote(self.model_name)} "

        output_path = f"/logs/agent/{self._OUTPUT_FILENAME}"

        # Run DeepSeek in non-interactive YOLO exec mode
        ...
        await self.exec_as_agent(
            environment,
            command=(
                f"deepseek exec --yolo --auto --stream-json "
    ...
    def populate_context_post_run(self, context: AgentContext) -> None:
        """Parse DeepSeek's output for any post-run metadata."""
        # DeepSeek writes its results to the working tree as git diffs.
