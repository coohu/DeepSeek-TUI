...
preflight_glibc() {
    ...
    if [[ "${DEEPSEEK_SKIP_GLIBC_CHECK:-}" == "1" || "${DEEPSEEK_TUI_SKIP_GLIBC_CHECK:-}" == "1" || "${DEEPSEEK_SKIP_GLIBC_CHECK:-}" == "1" ]]; then
        return 0
    fi
    ...
    if ! host="$(detect_host_glibc)" || [[ -z "$host" ]]; then
        ...
        echo "Build from source instead: cargo install deepseek-cli --locked && cargo install deepseek-tui --locked" >&2
        echo "Set DEEPSEEK_SKIP_GLIBC_CHECK=1 to bypass this check at your own risk." >&2
        return 1
    fi
...
for bin in deepseek codew deepseek-tui; do
    src="$SCRIPT_DIR/$bin"
    dst="$BIN_DIR/$bin"
    if [[ ! -f "$src" ]]; then
