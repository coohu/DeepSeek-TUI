...
# deepseek-* dependency pins), npm/deepseek/package.json (version +
...
# 2) Internal deepseek-* dependency pins in every crate manifest.
total = 0
for manifest in sorted(pathlib.Path("crates").glob("*/Cargo.toml")):
    text = manifest.read_text()
    out, n = re.subn(
        rf'(deepseek-[a-z0-9-]+\s*=\s*\{{[^}}]*version = "){old_re}(")',
        rf"\g<1>{new}\g<2>",
        text,
    )
    if n:
