...
Checks:
  ...
  - npm/deepseek release:check sees fresh npm-facing assets and checksum rows

Set GH_BIN=/path/to/gh to choose a GitHub CLI binary. Set
DEEPSEEK_GITHUB_REPO=owner/repo or DEEPSEEK_RELEASE_REMOTE=remote to override
the default coohu/deepseek-tui origin check.
EOF
}
...
repo="${DEEPSEEK_GITHUB_REPO:-coohu/deepseek-tui}"
remote="${DEEPSEEK_RELEASE_REMOTE:-origin}"
gh_bin="${GH_BIN:-gh}"
...
npm_package_version="$(node -p "require('./npm/deepseek/package.json').version")"
npm_binary_version="$(
  node -p "const p=require('./npm/deepseek/package.json'); p.codewhaleBinaryVersion || p.deepseekBinaryVersion || p.version"
)"
if [[ "${npm_package_version}" != "${version}" ]]; then
  echo "npm/deepseek package version ${npm_package_version} does not match ${version}." >&2
  exit 1
fi
if [[ "${npm_binary_version}" != "${version}" && "${allow_npm_binary_mismatch}" != "1" ]]; then
  echo "npm/deepseek codewhaleBinaryVersion ${npm_binary_version} does not match ${version}." >&2
...
(
  ...
  env \
    ...
    -u DEEPSEEK_RELEASE_BASE_URL \
    -u DEEPSEEK_USE_CNB_MIRROR \
    DEEPSEEK_TUI_VERSION="${version}" \
    DEEPSEEK_TUI_GITHUB_REPO="${repo}" \
    DEEPSEEK_ALLOW_NPM_BINARY_MISMATCH="${allow_npm_binary_mismatch}" \
    npm run release:check
)

echo "Release asset gate OK: ${tag} assets match ${local_sha} and npm/deepseek is ready for publish."
