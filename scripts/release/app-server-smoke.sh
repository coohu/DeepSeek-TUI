...
#
#   1. app-server stdio probe (always): drives `deepseek app-server --stdio`
...
#      from `deepseek auth list`, maps each to a cheap sentinel model, and
...
#   scripts/release/app-server-smoke.sh --bin ./target/release/deepseek
#
# Binary resolution order: --bin <path>, $DEEPSEEK_BIN, ./target/release/deepseek, PATH.
...
resolve_bin() {
    if [[ -n "$BIN" ]]; then
        [[ -x "$BIN" ]] || { echo "deepseek binary not executable: $BIN" >&2; exit 2; }
        return
    fi
    if [[ -x "./target/release/deepseek" ]]; then
        BIN="./target/release/deepseek"
    elif command -v deepseek >/dev/null 2>&1; then
        BIN="$(command -v deepseek)"
    else
        echo "could not find a deepseek binary." >&2
        echo "  build one: cargo build -p deepseek-cli --release" >&2
        echo "  or pass:   --bin <path> / DEEPSEEK_BIN=<path>" >&2
        exit 2
    fi
}
...
# Echo configured provider slugs (active != missing) from `deepseek auth list`.
