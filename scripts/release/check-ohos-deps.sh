...
# only asks Cargo to resolve the deepseek-tui dependency graph for the OHOS
...
package="${DEEPSEEK_OHOS_DEP_PACKAGE:-deepseek-tui}"

cargo_tree_with_retry() {
  local attempt
  local max_attempts="${DEEPSEEK_OHOS_DEP_RETRIES:-3}"
  local delay_seconds="${DEEPSEEK_OHOS_DEP_RETRY_DELAY_SECONDS:-10}"
  local err_file
  local output
  local status

  if ! [[ "${max_attempts}" =~ ^[0-9]+$ ]] || ((max_attempts < 1)); then
    echo "DEEPSEEK_OHOS_DEP_RETRIES must be an integer greater than or equal to 1." >&2
    return 1
  fi
