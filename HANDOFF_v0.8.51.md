...
- **Image attachment** (#2584/#2587): `ContentBlock::ImageUrl`, multimodal chat requests, base64 encoding, + exhaustive match arms across 15 files. Deferred by coohu — changes the request shape, needs multimodal endpoint testing.
...
cargo test -p deepseek-tui -- --test-threads=4
...
gh issue view 2600 --repo coohu/deepseek-tui  # re-read release checklist
```
...
- [ ] Full `cargo test --workspace` green — deepseek-tui validated; 1 environment-only Landlock test may fail on macOS
- [ ] `npm test` in `npm/deepseek`
...
2. **DeepSeek naming**: The branch and committed code use "DeepSeek" naming throughout. Do not imply DeepSeek is deprecated.
...
Generated by deepseek-v4-pro in DeepSeek v0.8.51 pre-release triage.
