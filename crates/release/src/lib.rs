...
pub const CHECKSUM_MANIFEST_ASSET: &str = "deepseek-artifacts-sha256.txt";

/// GitHub API URL for the single latest stable release.
pub const LATEST_RELEASE_URL: &str =
    "https://api.github.com/repos/coohu/deepseek-tui/releases/latest";

/// GitHub API URL listing recent releases (up to 100), used to find beta tags.
pub const RELEASES_URL: &str =
    "https://api.github.com/repos/coohu/deepseek-tui/releases?per_page=100";

/// Base URL of the DeepSeek repository on the CNB mirror platform.
pub const CNB_REPO_URL: &str = "https://cnb.cool/deepseek.net/deepseek";

/// Environment variable that overrides the base URL for release asset downloads.
pub const RELEASE_BASE_URL_ENV: &str = "DEEPSEEK_RELEASE_BASE_URL";
...
pub const CNB_MIRROR_ENV: &str = "DEEPSEEK_USE_CNB_MIRROR";
...
pub const UPDATE_USER_AGENT: &str = "deepseek-updater";

const CNB_RELEASE_ASSET_BASE: &str = "https://cnb.cool/coohu/deepseek-tui/-/releases";
...
/// CNB mirror if `DEEPSEEK_USE_CNB_MIRROR` is set. Returns `None` when no
        ...
        "GitHub release downloads may be blocked or slow on this network.\n\
         For mainland China, use one of these fallback paths:\n\
           1. Source build from the CNB mirror, installing both shipped binaries:\n\
              cargo install --git {CNB_REPO_URL} --tag vX.Y.Z deepseek-cli --locked --force\n\
...
mod tests {
    ...
    fn release_base_url_from_env_uses_cnb_mirror_last() {
        ...
        assert_eq!(
            ...
            Some("https://cnb.cool/coohu/deepseek-tui/-/releases/v1.2.3".to_string())
        );
