...
`deepseek serve --http` and `deepseek serve --mobile` remain compatibility
aliases for `deepseek app-server --http` and `deepseek app-server --mobile`.
New integrations should prefer `deepseek app-server`.")]
    ...
    #[command(after_help = "\
Transports:
  deepseek app-server --http              Full HTTP/SSE runtime API (/v1/*) on 127.0.0.1:7878
  deepseek app-server --mobile            Runtime API + phone control page (binds 0.0.0.0)
  deepseek app-server --stdio             JSON-RPC control transport over stdio (no listener)
  deepseek app-server                     Legacy in-process app-server HTTP on 127.0.0.1:8787

`--http` and `--mobile` serve the same mature runtime API as `deepseek serve
--http`/`--mobile`, which remain as compatibility aliases. The runtime API token
is read from --auth-token, DEEPSEEK_RUNTIME_TOKEN, or DEEPSEEK_RUNTIME_TOKEN.
...
/// Flags for `deepseek remote-setup`. Forwarded to the TUI binary, which owns
...
struct RemoteSetupArgs {
    ...
    /// Bundle output directory (default `./deepseek-deploy/<cloud>-<bridge>`).
...
struct AppServerArgs {
    ...
    /// API surface; it delegates to the same server as `deepseek serve --http`.
    #[arg(long, conflicts_with_all = ["stdio", "mobile"])]
    http: bool,
    /// Serve the runtime API plus the phone-friendly mobile control page.
    /// Equivalent to the legacy `deepseek serve --mobile`.
...
fn reject_exec_global_flags(args: &[String]) -> Result<()> {
    ...
    for arg in args {
        ...
        if GLOBAL_ONLY_FLAGS.contains(&flag) {
            bail!(
                "{flag} must be placed before `exec`.\n\nUse:\n  deepseek {flag} <value> exec \"<prompt>\""
            );
        }
    }
...
fn auth_status_all_providers(store: &ConfigStore, secrets: &Secrets) -> Vec<String> {
    ...
    lines.push(format!(
        "active provider: {} (set via config or DEEPSEEK_PROVIDER)",
    ...
    lines.push(String::new());
    lines.push("* = active provider (from config or DEEPSEEK_PROVIDER)".to_string());
...
/// `deepseek app-server --http`/`--mobile`. Maps app-server flags onto the
...
/// the runtime API reads DEEPSEEK_RUNTIME_TOKEN/DEEPSEEK_RUNTIME_TOKEN itself.
...
fn app_server_token_from_env() -> Option<String> {
    std::env::var("DEEPSEEK_APP_SERVER_TOKEN")
        .ok()
...
`` binary. To fix this, install one of:\n\
  • npm:    npm install -g             (downloads both binaries)\n\
  • cargo:  cargo install -cli  --locked\n\
  • GitHub Releases: download BOTH `deepseek-<platform>` AND \
`-<platform>` from https://github.com/coohu/DeepSeek-TUI/releases/latest \
...
mod tests {
    ...
    use deepseek_config::ProviderSource;
    ...
    fn model_command_provider_hint_uses_subcommand_then_top_level_provider() {
        ...
        let cli = parse_ok(&["deepseek", "--provider", "zai", "model", "list"]);
    ...
    fn serve_help_documents_forwarded_runtime_modes() {
        let help = help_for(&["deepseek", "serve", "--help"]);
    ...
    fn parses_direct_tui_command_aliases() {
        ...
        let cli = parse_ok(&["deepseek", "fleet", "init"]);
    ...
    fn cli_provider_helpers_follow_config_metadata() {
        let registry_kinds: Vec<ProviderKind> = deepseek_config::provider::all_providers()
            .iter()
    ...
    fn build_tui_command_forwards_explicit_openai_codex_provider() {
        ...
        let cli = parse_ok(&["deepseek", "--provider", "openai-codex", "doctor"]);
        let resolved = ResolvedRuntimeOptions {
            provider: ProviderKind::OpenaiCodex,
            provider_source: ProviderSource::Cli,
            model: "gpt-5.5".to_string(),
    ...
    fn build_tui_command_allows_anthropic_cli_provider() {
        ...
        let cli = parse_ok(&["deepseek", "--provider", "anthropic", "doctor"]);
    ...
    fn build_tui_command_forwards_provider_keyring_env_vars_for_all_providers() {
        ...
        for provider in ProviderKind::ALL {
            let cli = parse_ok(&["deepseek", "--workspace", "/tmp/deepseek-workspace"]);
            let resolved = ResolvedRuntimeOptions {
                provider,
                provider_source: ProviderSource::Config,
                model: "test-model".to_string(),
