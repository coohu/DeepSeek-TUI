...
#[given("an offline DeepSeek workspace containing:")]
fn offline_deepseek_workspace_containing(world: &mut ToolLifecycleWorld, step: &Step) {
...
async fn user_asks(world: &mut ToolLifecycleWorld, prompt: String) {
    ...
    let output = run_deepseek_exec(world, &server, &prompt);
...
#[then("DeepSeek should send the user request to the mocked LLM")]
fn deepseek_should_send_user_request_to_mocked_llm(world: &mut ToolLifecycleWorld) {
...
#[then("DeepSeek should send the tool result back to the mocked LLM")]
fn deepseek_should_send_tool_result_back_to_mocked_llm(world: &mut ToolLifecycleWorld) {
    let request = world
        .requests
        .iter()
...
#[then("DeepSeek should send the tool error back to the mocked LLM")]
fn deepseek_should_send_tool_error_back_to_mocked_llm(world: &mut ToolLifecycleWorld) {
    let request = world
        .requests
        .iter()
        .find(|request| request_contains_tool_result(request))
...
#[then("DeepSeek should send the malformed argument error back to the mocked LLM")]
fn deepseek_should_send_malformed_argument_error_back_to_mocked_llm(
    world: &mut ToolLifecycleWorld,
) {
    let request = world
        .requests
...
fn run_deepseek_exec(
    ...
    let mut command = Command::new(deepseek_tui_binary());
...
fn run_with_timeout(mut command: Command, timeout: Duration) -> std::process::Output {
    let mut child = command.spawn().expect("spawn deepseek-tui exec");
    ...
    let status = match child.wait_timeout(timeout).expect("wait for deepseek-tui") {
        Some(status) => status,
        None => {
            let _ = child.kill();
            let _ = child.wait();
            ...
            panic!(
                "deepseek-tui exec timed out after {timeout:?}\nstdout:\n{}\nstderr:\n{}",
...
fn deepseek_tui_binary() -> PathBuf {
    ...
    path.push(format!("deepseek-tui{}", std::env::consts::EXE_SUFFIX));
    path
}
