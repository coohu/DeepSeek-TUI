...
# cargo test -p deepseek-tui --bin deepseek-tui --features long-running-tests commands::groups::core::acceptance -- --test-threads=1
Feature: Core command visible surfaces

  Scenario: Core informational commands write visible transcript messages
    Given a DeepSeek core command workspace
    ...
    Then the message window should include "deepseek Home Dashboard"
  ...
  Scenario: Clear replaces prior transcript with visible confirmation
    Given a DeepSeek core command workspace with one visible user message
