...
#[given("an offline DeepSeek workspace with a configured plugin directory")]
...
async fn plugin_module_does_not_break_binary_load() {
    let output = Command::new(deepseek_tui_binary())
        .arg("--version")
        .output()
        .expect("deepseek-tui --version should start");

    assert!(
        output.status.success(),
        "deepseek-tui --version failed\nstderr:\n{}",
        String::from_utf8_lossy(&output.stderr)
    );

    let version = String::from_utf8_lossy(&output.stdout);
    assert!(
        version.contains("deepseek"),
        "version output should mention deepseek, got: {version}"
    );
}
