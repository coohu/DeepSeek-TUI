...
//!
//! This runs the binary through `deepseek-tui eval` and verifies that the
...
fn clean_deepseek_evaluation_workspace(world: &mut CoreSessionExtractionWorld) {
...
fn eval_harness_runs_shell_command(world: &mut CoreSessionExtractionWorld) {
    ...
    let output = Command::new(deepseek_tui_binary())
        .args([
            "eval",
            "--json",
            "--shell-command",
        ...
        .arg(record_dir.path())
        .output()
        .expect("deepseek-tui eval should start");

    assert!(
        output.status.success(),
        "deepseek-tui eval failed\nstderr:\n{}",
...
async fn deepseek_eval_runs_after_extraction() {
