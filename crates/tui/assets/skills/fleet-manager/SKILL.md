---
name: fleet-manager
description: Use when managing, triaging, restarting, escalating, or summarizing DeepSeek Agent Fleet runs and workers.
metadata:
  short-description: Triage DeepSeek Agent Fleet runs
---

# Fleet Manager

Use this skill when acting as a manager agent for DeepSeek Agent Fleet runs.
...
- Prefer typed fleet surfaces over shell spelunking: `deepseek fleet status`,
  `inspect`, `logs`, `artifacts`, `interrupt`, `restart`, `stop`, and the
  Runtime API fleet endpoints.
- Do not read `.deepseek/fleet.jsonl`, host logs, or remote files directly
...
1. Identify the run and worker from the user request, run receipt, or fleet
   status output. If no worker is named, start with `deepseek fleet status`.
2. Inspect the worker with `deepseek fleet inspect <worker-id>` or the matching
   Runtime API worker endpoint.
3. Review bounded evidence with `deepseek fleet logs <worker-id>` and
   `deepseek fleet artifacts <worker-id>`. Summarize artifact refs, not full
   payloads.
...
5. Choose one typed action:
   - transient and retry budget remains: `deepseek fleet restart <worker-id>`.
