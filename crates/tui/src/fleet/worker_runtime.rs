...
pub fn fleet_task_to_worker_spec(
    ...
    let max_spawn_depth = deepseek_config::FleetExecConfig::default().max_spawn_depth;
    let runtime_profile =
...
fn route_protocol_label(protocol: deepseek_config::route::RequestProtocol) -> &'static str {
    use deepseek_config::route::RequestProtocol;
    match protocol {
...
fn loadout_intent_label(loadout: &deepseek_config::FleetLoadout) -> Option<String> {
    if *loadout == deepseek_config::FleetLoadout::Inherit {
        None
    } else {
...
fn fleet_task_prompt_with_profile(
    ...
    prompt.push_str("You have been summoned as a DeepSeek Fleet member (");
...
fn effective_fleet_loadout(
    worker_profile: Option<&FleetTaskWorkerProfile>,
    agent_profile: Option<&AgentProfile>,
) -> deepseek_config::FleetLoadout {
    worker_profile
        .and_then(|worker| worker.model_class.as_deref().or(worker.loadout.as_deref()))
        .map(deepseek_config::FleetLoadout::from_name)
        .or_else(|| {
            agent_profile
                .map(|profile| profile.profile.loadout.clone())
                .filter(|loadout| *loadout != deepseek_config::FleetLoadout::Inherit)
        })
        .unwrap_or_default()
}
...
fn fleet_worker_runtime_profile_for_loadout(
    ...
    spawn_depth: u32,
    max_spawn_depth: u32,
    loadout: &deepseek_config::FleetLoadout,
) -> WorkerRuntimeProfile {
...
fn fleet_model_route_for_loadout(
    ...
    match loadout {
        deepseek_config::FleetLoadout::Inherit => ModelRoute::Inherit,
        deepseek_config::FleetLoadout::Fast => ModelRoute::Faster,
        deepseek_config::FleetLoadout::Strong
        | deepseek_config::FleetLoadout::Balanced
        | deepseek_config::FleetLoadout::DeepReasoning
        | deepseek_config::FleetLoadout::Code
        | deepseek_config::FleetLoadout::Review
        | deepseek_config::FleetLoadout::ToolHeavy
        | deepseek_config::FleetLoadout::Custom(_) => ModelRoute::Auto,
    }
}

/// Create a fleet artifact ref from a worker output.
///
/// Uses the fleet artifact conventions: logs go under `.deepseek/fleet/`,
/// reports under `.deepseek/fleet/reports/`.
pub fn fleet_artifact_ref(
    _run_id: &str,
    _worker_id: &str,
    kind: deepseek_protocol::fleet::FleetArtifactKind,
    path: std::path::PathBuf,
) -> deepseek_protocol::fleet::FleetArtifactRef {
    deepseek_protocol::fleet::FleetArtifactRef {
        kind,
        path,
        checksum: None,
...
pub fn apply_exec_hardening(
    ...
    exec: &deepseek_config::FleetExecConfig,
) -> AgentWorkerSpec {
...
mod tests {
    ...
    fn agent_profile(
        ...
        AgentProfile {
            ...
            profile: deepseek_config::FleetProfile {
                slot: deepseek_config::FleetSlot::from_name(role),
                role: deepseek_config::FleetRole {
                    name: role.to_string(),
                ...
                loadout,
                model: None,
                permissions: deepseek_config::FleetProfilePermissions::default(),
                delegation: deepseek_config::FleetDelegationHints::default(),
            },
    ...
    fn fleet_task_prompt_includes_instructions_context_and_input_files() {
        ...
        assert!(prompt.contains("summoned as a DeepSeek Fleet member (general)"));
    ...
    fn fleet_worker_spec_resolves_agent_profile_role_prompt_and_loadout() {
        ...
        assert!(
            spec.objective
                .contains("summoned as a DeepSeek Fleet member (reviewer)")
        );
    ...
    fn fleet_worker_spec_defaults_to_shared_subagent_depth() {
        ...
        assert_eq!(spec.max_spawn_depth, deepseek_config::DEFAULT_SPAWN_DEPTH);
        ...
        let hardened = apply_exec_hardening(spec, &deepseek_config::FleetExecConfig::default());
    ...
    fn fleet_route_parity_uses_shared_router_candidates() {
        ...
        assert_eq!(
            fleet_model_route_for_loadout("auto", &deepseek_config::FleetLoadout::Fast),
            ModelRoute::Faster,
        );
        assert_eq!(
            fleet_model_route_for_loadout("auto", &deepseek_config::FleetLoadout::Inherit),
            ModelRoute::Inherit,
        );
        assert_eq!(
            fleet_model_route_for_loadout("auto", &deepseek_config::FleetLoadout::Strong),
            ModelRoute::Auto,
        );
    ...
    fn exec_hardening_caps_max_steps_to_max_turns() {
        ...
        let exec = deepseek_config::FleetExecConfig {
            max_turns: 50,
            ..Default::default()
        };
