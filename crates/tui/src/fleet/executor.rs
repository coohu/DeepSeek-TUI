//! Fleet executor — runs a fleet worker as a real `deepseek exec` subprocess.
//!
//! A fleet worker IS a headless `deepseek exec` run. There is no separate
//! "fleet worker" execution engine: the sub-agent runtime, full tool surface,
//! and recursion depth all come from the one `deepseek exec` runtime, so
...
//!   into the `deepseek exec --output-format stream-json …` argv that a host
...
use anyhow::Result;
use deepseek_config::FleetExecConfig;
use deepseek_protocol::fleet::{FleetHostSpec, FleetTaskSpec, FleetWorkerEventPayload};
...
/// Build the `deepseek exec` argv that runs a fleet task headlessly.
///
...
/// (`[fleet.exec] max_spawn_depth`, default [`deepseek_config::DEFAULT_SPAWN_DEPTH`]).
///
...
fn build_worker_exec_command_from_prompt(
    ...
    FleetWorkerCommand::new(deepseek_binary.to_string(), args)
}

/// Map one `deepseek exec` stream-json line into a fleet ledger event.
///
...
/// Drives fleet workers as real `deepseek exec` subprocesses on the local
/// host, incrementally draining each worker's stream-json output into fleet
/// ledger events.
///
/// The caller (the `deepseek fleet run` loop / `FleetManager`) owns the
...
mod tests {
    ...
    use deepseek_protocol::fleet::{FleetTaskSpec, FleetTaskWorkerProfile};
    ...
    #[test]
    fn worker_command_is_a_headless_deepseek_exec_run() {
        let exec = FleetExecConfig::default();
        let cmd = build_worker_exec_command("deepseek", &task("read the file"), &exec, None);
        assert_eq!(cmd.program, "deepseek");
    ...
    fn worker_command_threads_exec_hardening_flags() {
        ...
        let cmd = build_worker_exec_command("deepseek", &task("audit"), &exec, Some("glm-5.1"));
        let joined = cmd.args.join(" ");
    ...
    fn unbounded_max_turns_is_not_passed() {
        ...
        let cmd = build_worker_exec_command("deepseek", &task("x"), &exec, None);
    ...
    /// `deepseek exec`), and prove the executor drains its events and terminal
    /// exit through the real host adapter — no deepseek binary needed. This is
    ...
    /// passing ones — all without the deepseek binary.
    #[cfg(unix)]
    #[test]
