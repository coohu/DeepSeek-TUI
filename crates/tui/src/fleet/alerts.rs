...
impl FleetAlertEvent {
    ...
    pub fn inspection_commands(&self) -> Vec<String> {
        let mut commands = vec!["deepseek fleet status".to_string()];
        if let Some(worker_id) = &self.worker_id {
            commands.push(format!("deepseek fleet inspect {worker_id}"));
        }
        commands
    }
}
...
fn slack_body(event: &FleetAlertEvent, channel: Option<&str>) -> Value {
    let text = format!(
        "DeepSeek fleet {}: run={} task={} reason={}",
...
fn pagerduty_body(event: &FleetAlertEvent, severity: &str, routing_key: String) -> Value {
    json!({
        ...
        "payload": {
            "summary": format!("DeepSeek fleet {}: {}", alert_class_label(event.class), short_reason(&event.reason)),
            "severity": severity,
            "source": "deepseek",
...
fn redacted_secret_header(secret_env: Option<&str>) -> Value {
    match secret_env {
        Some(name) => json!({ "X-DeepSeek-Webhook-Secret": redacted_env(name) }),
        None => json!({}),
    }
}
...
mod tests {
    use super::*;
    use deepseek_protocol::fleet::{FleetScore, FleetTaskResult};
    ...
    fn fleet_alert_dry_run_redacts_secrets() {
        ...
        assert!(payload.contains("deepseek fleet inspect worker-1"));
    }
    ...
    fn fleet_alert_event_is_derived_from_ledgered_stale_worker_event() {
        ...
        assert_eq!(
            ...
            vec![
                "deepseek fleet status".to_string(),
                "deepseek fleet inspect worker-1".to_string()
            ]
        );
    }
