...
pub struct PromptSessionContext<'a> {
    ...
    /// facts. Defaults to `"deepseek"` when the caller doesn't supply one.
    pub model_id: &'a str,
    ...
    /// Restrict skill discovery to DeepSeek-owned roots plus explicit
    /// `skills_dir` configuration.
    pub skills_scan_deepseek_only: bool,
}
...
impl Default for PromptSessionContext<'_> {
    fn default() -> Self {
        Self {
            ...
            show_thinking: true,
            verbosity: None,
            skills_scan_deepseek_only: false,
        }
    }
}
...
/// Compaction Relay stay owned by DeepSeek's system prompt assembly.
#[non_exhaustive]
#[derive(Debug)]
...
/// Embedder hook for replacing DeepSeek's byte-stable base/personality prompt
/// segment.
...
// optional file in the DeepSeek config directory. This lets users repurpose
...
// also set an explicit opt-in flag (`DEEPSEEK_ALLOW_BASE_PROMPT_OVERRIDE`).
...
pub const BASE_PROMPT_OVERRIDE_OPT_IN_ENV: &str = "DEEPSEEK_ALLOW_BASE_PROMPT_OVERRIDE";
...
/// Resolve the DeepSeek config directory and load any prompt overrides found
...
pub fn load_prompt_overrides_from_config_home() {
    let Ok(home) = deepseek_config::deepseek_home() else {
        return;
    };
...
Bạn đang chạy trong deepseek. Cho dù ngữ cảnh tác vụ (mã nguồn, nhật ký lỗi, tên tệp) \
...
The Constitution of DeepSeek governs your behavior. Ground truth is the
...
pub fn compose_prompt(personality: Personality) -> String {
    compose_prompt_with_approval_model_and_shell(personality, "deepseek")
}
...
pub fn system_prompt_for_mode_with_context_skills_session_and_approval(
    ...
    // conventions show up in the catalogue. Users can opt into a DeepSeek-only
    // scan with `[skills] scan_deepseek_only = true`. When an explicit
    ...
    let skill_discovery_mode = crate::skills::SkillDiscoveryMode::from_deepseek_only(
        session_context.skills_scan_deepseek_only,
    );
...
mod tests {
    ...
    fn authority_recap_appears_in_full_prompt() {
        ...
        assert!(
            text.contains("The Constitution of DeepSeek governs your behavior"),
