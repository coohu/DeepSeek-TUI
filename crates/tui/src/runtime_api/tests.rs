...
fn workspace_status_reports_head_and_dirty_counts() -> Result<()> {
    ...
    run_test_git(
        ...
        &[
            ...
            "-c",
            "user.email=deepseek@example.invalid",
            "commit",
            "-m",
            "init",
        ],
...
fn token_from_cookie_header_decodes_percent_encoded_token() {
    assert_eq!(
        token_from_cookie_header(Some(
            "theme=dark; deepseek_runtime_token=abc%20ABC%2B%2F%3F%3A%3D%26%25"
        )),
        Some("abc ABC+/?:=&%".to_string())
    );
    assert_eq!(
        token_from_cookie_header(Some("deepseek_runtime_token=bad%ZZ")),
        None
    );
}
...
> {
    let root = std::env::temp_dir().join(format!("deepseek-config-api-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
...
async fn health_and_tasks_endpoints_work() -> Result<()> {
    ...
    assert_eq!(health["service"], "deepseek-runtime-api");
...
async fn mcp_tools_endpoint_is_passive_until_connect_requested() -> Result<()> {
    let root = std::env::temp_dir().join(format!("deepseek-mcp-tools-api-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    fs::create_dir_all(&root)?;
...
async fn runtime_token_guard_protects_v1_routes() -> Result<()> {
    ...
    let cookie_token = client
        ...
        .header(
            header::COOKIE,
            format!("deepseek_runtime_token={}", url_query_component(&token)),
        )
        .send()
        .await?
        .error_for_status()?;
...
async fn fleet_status_runtime_api_exposes_state_and_actions() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-fleet-api-{}", Uuid::new_v4()));
    let workspace = root.join("workspace");
    fs::create_dir_all(&workspace)?;
    let manager = FleetManager::open(&workspace)?;
    let task = deepseek_protocol::fleet::FleetTaskSpec {
        id: "task-a".to_string(),
        name: "Task A".to_string(),
        description: None,
        objective: Some("Inspect fleet status through Runtime API".to_string()),
        instructions: "Stay running for inspection.".to_string(),
        worker: Some(deepseek_protocol::fleet::FleetTaskWorkerProfile {
            agent_profile: None,
    ...
    let sub_agent_manager = runtime_api_sub_agent_manager(&workspace, 2);
    {
        ...
        guard.register_worker(AgentWorkerSpec {
            ...
            max_steps: 8,
            spawn_depth: 0,
            max_spawn_depth: deepseek_config::DEFAULT_SPAWN_DEPTH,
        });
    }
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root_token_mobile_workspace_and_subagents(
...
async fn agent_runs_runtime_api_exposes_persisted_worker_receipts() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-agent-runs-api-{}", Uuid::new_v4()));
    let workspace = root.join("workspace");
    fs::create_dir_all(workspace.join(".deepseek/state"))?;
    ...
    fs::write(
        workspace.join(".deepseek/state/subagents.v1.json"),
...
async fn runtime_info_reports_bind_state() -> Result<()> {
    ...
    let info: serde_json::Value = client
        ...
        .json()
        .await?;
    assert_eq!(info["service"], "deepseek-runtime-api");
    assert_eq!(info["runtime_api_version"], "1.0");
    assert_eq!(info["deepseek_version"], info["version"]);
...
async fn mobile_page_is_available_only_when_enabled() -> Result<()> {
    ...
    assert!(html.contains("DeepSeek Mobile"));
...
async fn mobile_page_serves_shell_when_auth_enabled() -> Result<()> {
    ...
    let bearer = client
        ...
        .await?
        .error_for_status()?;
    assert!(bearer.text().await?.contains("DeepSeek Mobile"));
...
async fn mobile_insecure_mode_allows_page_and_v1_routes_without_token() -> Result<()> {
    ...
    let page = client
        ...
        .await?
        .error_for_status()?;
    assert!(page.text().await?.contains("DeepSeek Mobile"));
...
#[test]
fn resolve_skills_dir_respects_deepseek_only_scan() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let workspace = tmp.path();
    let agents_skills = workspace.join(".agents").join("skills");
    let deepseek_skills = workspace.join(".deepseek").join("skills");
    fs::create_dir_all(&agents_skills).expect("create agents skills dir");
    fs::create_dir_all(&deepseek_skills).expect("create deepseek skills dir");

    let config = Config {
        skills: Some(crate::config::SkillsConfig {
            scan_deepseek_only: Some(true),
            ..Default::default()
        }),
        ..Default::default()
    };
    ...
    let expected = fs::canonicalize(&deepseek_skills).expect("canonical deepseek skills");
    assert_eq!(resolved, expected);
}

#[test]
fn resolve_skills_dir_preserves_explicit_dir_in_deepseek_only_scan() {
    let tmp = tempfile::tempdir().expect("tempdir");
...
#[cfg(unix)]
#[test]
fn resolve_skills_dir_rejects_deepseek_only_symlink_escaping_workspace() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let workspace_root = tmp.path().join("workspace");
    let escape_target = tmp.path().join("escape_target");
    fs::create_dir_all(&workspace_root).expect("create workspace");
    ...
    let dotcodewhale = workspace_root.join(".deepseek");
    fs::create_dir_all(&dotcodewhale).expect("create .deepseek");
    ...
    assert_ne!(
        resolved, canon_escape,
        "DeepSeek-only symlink escaping workspace must not be resolved as skills dir"
    );
    assert_eq!(
        resolved,
        config.skills_dir(),
        "with no valid in-workspace DeepSeek skills dir, resolution should fall back to config"
    );
}
...
async fn set_config_rejects_unknown_key_with_bad_request() -> Result<()> {
    let root = std::env::temp_dir().join(format!("deepseek-config-unknown-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_validates_max_history_input() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-maxhist-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_validates_subagents_enabled_input() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-subenabled-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_validates_subagents_max_depth_input() -> Result<()> {
    ...
    // values with a descriptive 400 error.
    let root = std::env::temp_dir().join(format!("deepseek-config-subdepth-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_with_config_path_writes_to_specified_file() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("deepseek-config-path-persist-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    ...
    // clamping (Fix #1).
    let over_ceiling = u64::from(deepseek_config::MAX_SPAWN_DEPTH_CEILING) + 10;
    ...
    let expected = format!(
        "max_depth = {}",
        u64::from(deepseek_config::MAX_SPAWN_DEPTH_CEILING)
    );
    assert!(
...
async fn reload_config_endpoint_returns_success() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-reload-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn reload_config_reads_from_config_path_and_updates_in_memory_state() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-reload-path-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
    ...
    assert_eq!(
        initial_depth,
        u64::from(deepseek_config::DEFAULT_SPAWN_DEPTH),
...
async fn reload_config_refreshes_mcp_config_path() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-mcp-refresh-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn set_config_with_persist_false_does_not_write_to_disk() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-nopersist-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
...
async fn set_config_subagents_max_depth_below_ceiling_not_clamped() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("deepseek-config-depth-noclamp-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
    ...
    let ceiling = u64::from(deepseek_config::MAX_SPAWN_DEPTH_CEILING);
    let (status, body) = post_set_config(
        &client,
        &addr,
        "subagents_max_depth",
...
async fn set_config_subagents_enabled_false_persists() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-subfalse-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
...
async fn reload_config_with_malformed_file_returns_error() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-malformed-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn reload_config_applies_multiple_persisted_keys() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-multi-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn set_config_response_contains_all_expected_fields() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("deepseek-config-shape-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
