...
pub(crate) fn deepseek_home_dir() -> Option<PathBuf> {
    std::env::var_os("DEEPSEEK_HOME").and_then(|path| {
...
pub(crate) fn home_config_path() -> Option<PathBuf> {
    if let Some(home) = deepseek_home_dir() {
    ...
    effective_home_dir().map(|home| {
        ...
        let legacy = home.join(".codewhale").join("config.toml");
        if legacy.exists() {
            return legacy;
        }
        primary
...
pub(crate) fn default_mcp_config_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| {
        ...
        let legacy = home.join(".codewhale").join("mcp.json");
        if legacy.exists() {
            return legacy;
        }
        primary
...
pub(crate) fn default_notes_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| {
        ...
        let legacy = home.join(".codewhale").join("notes.txt");
        if legacy.exists() {
            return legacy;
        }
        primary
...
pub(crate) fn default_memory_path() -> Option<PathBuf> {
    effective_home_dir().map(|home| {
        ...
        let legacy = home.join(".codewhale").join("memory.md");
        if legacy.exists() {
            return legacy;
        }
        primary
