...
fn api_provider_metadata_helpers_follow_config_provider_metadata() {
    ...
    for kind in deepseek_config::ProviderKind::ALL {
...
fn provider_config_key_follows_config_provider_metadata() {
    for kind in deepseek_config::ProviderKind::ALL
        .into_iter()
        .filter(|kind| *kind != deepseek_config::ProviderKind::Deepseek)
    {
        let provider = ApiProvider::from_kind(kind);
        assert_eq!(
...
fn config_loads_sibling_permissions_into_exec_policy_engine() {
    ...
    fs::write(
        dir.path().join(deepseek_config::PERMISSIONS_FILE_NAME),
        r#"
[[rules]]
tool = "exec_shell"
command = "cargo test"
"#,
    ...
    let decision = config
        ...
        .check(deepseek_execpolicy::ExecPolicyContext {
            ...
            path: None,
            ask_for_approval: deepseek_execpolicy::AskForApproval::OnFailure,
            sandbox_mode: None,
        })
...
#[test]
fn load_honors_deepseek_home_for_primary_config_path() -> Result<()> {
    let _lock = lock_test_env();
    let dir = tempfile::tempdir()?;
    let deepseek_home = dir.path().join("isolated-deepseek");
    fs::create_dir_all(&deepseek_home)?;
    fs::write(deepseek_home.join("config.toml"), "provider = \"zai\"\n")?;
    let _deepseek_home = EnvVarGuard::set("DEEPSEEK_HOME", deepseek_home.as_os_str());
    let _deepseek_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
    let _deepseek_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");

    let expected = deepseek_home.join("config.toml");
...
fn profile_hotbar_override_replaces_entire_user_list() {
    ...
    profiles.insert(
        "compact".to_string(),
        Config {
            hotbar: Some(vec![deepseek_config::HotbarBindingToml {
                slot: 2,
    ...
    assert_eq!(
        merged.hotbar,
        Some(vec![deepseek_config::HotbarBindingToml {
            slot: 2,
            action: "session.compact".to_string(),
            label: Some("Compact".to_string()),
        }])
...
fn verifier_config_parses_hunt_policy_and_merges_overrides() {
    ...
    assert_eq!(
        ...
        deepseek_config::VerifierVerdictPolicy::Hunt
    );

    let merged = merge_config(
        Config {
            verifier: Some(deepseek_config::VerifierConfigToml {
                enabled: false,
                verdict_policy: deepseek_config::VerifierVerdictPolicy::Hunt,
            }),
            ..Config::default()
        },
        Config {
...
fn apply_env_overrides_sets_search_base_url() {
    let _guard = lock_test_env();
    let prev_codewhale = env::var_os("DEEPSEEK_SEARCH_BASE_URL");
    let prev_deepseek = env::var_os("DEEPSEEK_SEARCH_BASE_URL");
    unsafe {
        env::remove_var("DEEPSEEK_SEARCH_BASE_URL");
        env::set_var(
    ...
    unsafe {
        EnvGuard::restore_var("DEEPSEEK_SEARCH_BASE_URL", prev_codewhale);
...
impl EnvGuard {
    fn new(home: &Path) -> Self {
        ...
        let deepseek_home_prev = env::var_os("DEEPSEEK_HOME");
        ...
        let deepseek_provider_prev = env::var_os("DEEPSEEK_PROVIDER");
        let deepseek_model_prev = env::var_os("DEEPSEEK_MODEL");
        let deepseek_base_url_prev = env::var_os("DEEPSEEK_BASE_URL");
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            Self::restore_var("DEEPSEEK_HOME", self.deepseek_home.take());
...
fn subagent_max_spawn_depth_defaults_allows_zero_and_clamps() {
    assert_eq!(
        ...
        deepseek_config::DEFAULT_SPAWN_DEPTH
    );
    ...
    let high = Config {
        subagents: Some(SubagentsConfig {
            max_depth: Some(deepseek_config::MAX_SPAWN_DEPTH_CEILING + 10),
            ..SubagentsConfig::default()
        }),
        ..Config::default()
    };
    assert_eq!(
        ...
        deepseek_config::MAX_SPAWN_DEPTH_CEILING
    );
}
...
#[test]
fn skills_scan_deepseek_only_defaults_false_and_parses_true() -> Result<()> {
    assert!(!Config::default().skills_config().scan_deepseek_only());

    let config: Config = toml::from_str(
        r#"
[skills]
scan_deepseek_only = true
"#,
    )?;

    assert!(config.skills_config().scan_deepseek_only());
    Ok(())
}
...
fn profile_skills_config_merges_individual_fields() {
    ...
    assert_eq!(skills.scan_deepseek_only, Some(true));
}
...
fn nvidia_nim_provider_normalizes_deepseek_v4_flash_alias() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-nim-flash-model-alias-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn vllm_env_resolves_reported_lan_http_endpoint_and_model() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-vllm-lan-http-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn arcee_provider_uses_direct_defaults() -> Result<()> {
    ...
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "arcee");
...
fn siliconflow_cn_base_url_env_normalizes_model_aliases() -> Result<()> {
    ...
    // Safety: test-only environment mutation guarded by a global mutex.
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "siliconflow-CN");
        env::set_var("SILICONFLOW_API_KEY", "sf-env-key");
...
fn siliconflow_cn_reads_hyphenated_provider_table_from_config_file() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-siliconflow-cn-table-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_cn_preserves_reported_deepseek_prefixed_v4_route() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-siliconflow-cn-v4-report-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_cn_falls_back_to_shared_siliconflow_table_when_unset() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-siliconflow-cn-fallback-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_cn_env_overrides_write_cn_table_only() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-siliconflow-cn-env-table-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn moonshot_kimi_oauth_reads_kimi_code_home_credential() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-kimi-code-oauth-key-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
/// Env-var-only path: `DEEPSEEK_BASE_URL=https://api.kimi.com/coding/v1`
/// combined with `DEEPSEEK_PROVIDER=moonshot` must trigger Kimi Code
...
fn moonshot_kimi_code_env_base_url_selects_coding_model() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-kimi-code-env-url-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
"#,
    ...
    unsafe {
        ...
        env::set_var("DEEPSEEK_BASE_URL", "https://api.kimi.com/coding/v1");
    }
...
/// `DEEPSEEK_PROVIDER` twin below for the public env path.
#[test]
fn moonshot_kimi_code_model_overrides_root_deepseek_default() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-kimi-code-root-model-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
/// Same regression as above, but driven by the public `DEEPSEEK_PROVIDER`
/// env var. Documents the recommended user-facing setup path: never
/// `DEEPSEEK_PROVIDER=moonshot`, always `DEEPSEEK_PROVIDER=moonshot`
...
fn moonshot_platform_defaults_to_kimi_k27_code() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-moonshot-platform-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn save_api_key_for_openrouter_writes_provider_table() -> Result<()> {
    ...
    let _guard = EnvGuard::new(&temp_root);
    let config_path = temp_root.join(".deepseek").join("config.toml");
    let _config_path = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", config_path.as_os_str());
...
fn huggingface_provider_uses_direct_defaults() -> Result<()> {
    ...
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "huggingface");
...
fn huggingface_missing_key_error_mentions_env_fallbacks() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-huggingface-missing-key-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn huggingface_short_env_fallbacks_configure_route() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "deepseek-tui-huggingface-short-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
    ...
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "hf");
        env::set_var("HF_TOKEN", "hf-token-value");
