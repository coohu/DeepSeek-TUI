...
pub(crate) fn log_directory() -> Option<PathBuf> {
    ...
    // check the env var directly rather than deepseek_home()'s Ok/Err because
    // that helper succeeds (returns $HOME/.deepseek) even when the override is
...
mod tests {
    ...
    #[test]
    fn log_directory_honors_deepseek_home_as_hard_override() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempfile::TempDir::new().unwrap();
        // SAFETY: serialised by lock_test_env.
        unsafe {
        ...
        // $DEEPSEEK_HOME IS the home dir (no ".deepseek" appended), and the
