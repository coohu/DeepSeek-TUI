...
//! (`~/.deepseek/snapshots` or legacy `~/.deepseek/snapshots`) with
...
/// `$STATE_DIR` is resolved via `deepseek_config::resolve_state_dir`.
...
fn snapshot_base_with_home(home: Option<PathBuf>) -> PathBuf {
    ...
    let primary = home.join(".deepseek").join("snapshots");
    if primary.exists() {
        return primary;
    }
