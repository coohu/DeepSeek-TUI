...
mod tests {
    ...
    fn persist_status_items_writes_tui_section_to_config_toml() {
        let temp_root = temp_root("deepseek-statusline-persist");
    ...
    #[test]
    fn config_toml_path_uses_deepseek_home_for_fresh_installs() {
        let temp_root = temp_root("deepseek-config-path-fresh");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_toml_path_preserves_legacy_config_when_it_exists() {
        let temp_root = temp_root("deepseek-config-path-legacy");
