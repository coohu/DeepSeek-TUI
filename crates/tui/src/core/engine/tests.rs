...
fn structured_state_block_uses_checklist_as_work_surface() {
    let state = StructuredState {
        ...
        workspace: PathBuf::from("/workspace/deepseek"),
        cwd: Some(PathBuf::from("/workspace/deepseek")),
        working_set_summary: None,
...
fn ask_rule_engine(command: &str) -> deepseek_execpolicy::ExecPolicyEngine {
    deepseek_execpolicy::ExecPolicyEngine::with_rulesets(vec![
        deepseek_execpolicy::Ruleset::user(vec![], vec![])
            .with_ask_rules(vec![deepseek_execpolicy::ToolAskRule::exec_shell(command)]),
    ])
}

fn file_ask_rule_engine(tool: &str, path: &str) -> deepseek_execpolicy::ExecPolicyEngine {
    deepseek_execpolicy::ExecPolicyEngine::with_rulesets(vec![
        deepseek_execpolicy::Ruleset::user(vec![], vec![]).with_ask_rules(vec![
            deepseek_execpolicy::ToolAskRule::file_path(tool, path),
        ]),
    ])
}
    ...
    let (engine, handle) = Engine::new(EngineConfig::default(), &Config::default());
    {
        ...
        goal.mark_complete(
            ...
            crate::tools::goal::GoalCompletionVerification {
                ...
                check: "cargo test -p deepseek-tui runtime_goal_updates_emit_ui_snapshot"
                    .to_string(),
...
fn route_context_budget_prefers_resolved_route_limits() {
    let _lock = lock_test_env();
    let limits = deepseek_config::route::RouteLimits {
