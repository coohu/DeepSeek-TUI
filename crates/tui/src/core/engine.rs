...
use anyhow::Result;
use deepseek_execpolicy::{AskForApproval, ExecPolicyContext};
use deepseek_protocol::runtime::DynamicToolSpec;
...
pub struct EngineConfig {
    ...
    pub active_route_limits: Option<deepseek_config::route::RouteLimits>,
    ...
    /// `[subagents] max_depth = N` in `~/.deepseek/config.toml`.
...
impl Default for EngineConfig {
    fn default() -> Self {
        Self {
            ...
            exec_policy_engine: deepseek_execpolicy::ExecPolicyEngine::new(Vec::new(), Vec::new()),
        }
    }
}
...
pub struct Engine {
    ...
    active_route_limits: Option<deepseek_config::route::RouteLimits>,
...
impl Engine {
        ...
        let system_prompt =
            prompts::system_prompt_for_mode_with_context_skills_session_and_approval(
                ...
                prompts::PromptSessionContext {
                    ...
                    skills_scan_deepseek_only: config.skills_scan_deepseek_only,
                },
            );
                ...
                EngineRunInput::Operation(op) => match op {
                    ...
                    Op::SetSubagentRuntimeConfig {
                        ...
                        self.config.max_spawn_depth =
                            max_spawn_depth.min(deepseek_config::MAX_SPAWN_DEPTH_CEILING);
                ...
                (
                    ...
                    Some(format!(
                        "The engine hit an internal error and stopped this turn: {detail}. \
                         ...
                         A crash report was saved to ~/.deepseek/crashes/."
                    )),
                )
            }
        };
    ...
    fn build_tool_context(&self, mode: AppMode, auto_approve: bool) -> ToolContext {
        ...
        .with_skills_config(
            ...
            self.config.skills_scan_deepseek_only,
        )
    ...
    fn refresh_system_prompt(&mut self) {
        ...
        let base = prompts::system_prompt_for_mode_with_context_skills_session_and_approval(
            ...
            prompts::PromptSessionContext {
                ...
                skills_scan_deepseek_only: self.config.skills_scan_deepseek_only,
            },
        );
