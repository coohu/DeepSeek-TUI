...
impl WorkingSet {
    ...
    /// `DEEPSEEK_CACHE_MAXIMAL` env toggle (`1`/`true`/`on`/`yes`). The env
    ...
    fn cache_maximal_enabled(&self) -> bool {
        ...
        match std::env::var("DEEPSEEK_CACHE_MAXIMAL") {
            Ok(v) => matches!(
...
mod tests {
    ...
    // don't touch the process-wide `DEEPSEEK_CACHE_MAXIMAL` env var (which
