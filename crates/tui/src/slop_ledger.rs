...
//! ## Design
//!
//! - **Storage**: `~/.deepseek/slop_ledger.json` (a JSON array of entries).
...
impl SlopLedger {
    /// Resolve the default ledger path under the primary `~/.deepseek` root
    /// (with one-time legacy migration) so loads and saves never perpetuate
    /// `~/.deepseek` (#3240).
    pub fn default_path() -> io::Result<PathBuf> {
        deepseek_config::ensure_state_dir("slop_ledger")
    ...
    pub fn save(&self) -> io::Result<()> {
        ...
        // ~/.deepseek root (with one-time legacy migration), so persisting
...
/// to the portable `~/.deepseek/secrets` form.
fn redact_exported_text(text: &mut String) {
    ...
    if let Some(home) = dirs::home_dir() {
        for leaf in [".deepseek/secrets", ".deepseek/secrets"] {
            let dir = home.join(leaf);
            let prefix = dir.to_string_lossy().to_string();
            result = result.replace(&prefix, "~/.deepseek/secrets");
        }
    }
    *text = result;
}
