...
pub async fn run_http_server(
    ...
    let runtime_token_env = std::env::var("DEEPSEEK_RUNTIME_TOKEN")
        .ok()
...
fn print_mobile_urls(addr: SocketAddr, auth_enabled: bool, generated_auth: bool, show_qr: bool) {
    ...
    if auth_enabled {
        if generated_auth {
            println!(
                "  Auth uses an unprinted generated token; restart with DEEPSEEK_RUNTIME_TOKEN or --auth-token to sign in from another client."
            );
        } else {
...
fn fleet_artifact_json(artifact: &deepseek_protocol::fleet::FleetArtifactRef) -> Value {
    json!({
...
fn fleet_event_json(event: &deepseek_protocol::fleet::FleetWorkerEvent) -> Value {
    json!({
        "seq": event.seq,
...
async fn list_skills(
    ...
    let (skills_dir, mode) = {
        ...
        let mode = crate::skills::SkillDiscoveryMode::from_deepseek_only(
            config.skills_config().scan_deepseek_only(),
        );
        (skills_dir, mode)
    };
...
fn resolve_skills_dir(config: &Config, workspace: &std::path::Path) -> PathBuf {
    if config.skills_config().scan_deepseek_only() {
        if config.skills_dir.is_some() {
            return config.skills_dir();
        }
        if let Some(deepseek_skills_dir) = crate::skills::deepseek_workspace_skills_dir(workspace)
            && let Ok(canonical_skills) = fs::canonicalize(&deepseek_skills_dir)
        {
    ...
    if persist {
        ...
        let result: anyhow::Result<PathBuf> = match key.as_str() {
            ...
            "subagents_max_depth" => {
                ...
                let clamped = raw.min(u64::from(deepseek_config::MAX_SPAWN_DEPTH_CEILING));
