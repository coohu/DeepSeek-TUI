...
impl HooksConfig {
    /// Load global hooks merged with project-local `.deepseek/hooks.toml` (#3026).
    ///
    ...
    pub fn load_with_project(global: HooksConfig, workspace: &Path) -> HooksConfig {
        let project_path = workspace.join(".deepseek").join("hooks.toml");
...
mod tests {
    ...
    fn trust_workspace_for_project_hooks(workspace: &Path, config_path: &Path) -> EnvVarGuard {
        let guard = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", config_path);
...
"#;
    ...
    fn turn_end_payload_contains_post_turn_observer_fields() {
        let context = HookContext::new()
            ...
            .with_workspace(PathBuf::from("/tmp/deepseek"))
            .with_mode("agent")
        ...
        assert_eq!(payload["workspace"], "/tmp/deepseek");
...
"#,
    ...
    fn load_with_project_ignores_project_hooks_until_workspace_trusted() {
        ...
        let dir = tempfile::tempdir().expect("tempdir");
        let _config = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", dir.path().join("config.toml"));
        let _legacy_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
        let project_dir = dir.path().join(".deepseek");
        std::fs::create_dir_all(&project_dir).expect("mkdir .deepseek");
        std::fs::write(
