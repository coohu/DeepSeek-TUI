...
pub fn model(app: &mut App, model_name: Option<&str>) -> CommandResult {
    if let Some(name) = model_name {
        ...
        // Reject a model that is stored in provider_models for a different provider.
        // This catches e.g. "kimi-k2.6" being attempted on Deepseek when it was
        // last used with Moonshot.
        if !app.accepts_custom_model_ids() {
            let active_key = app.api_provider.as_str();
            let is_other_provider_model = app
                .provider_models
                .iter()
                .any(|(k, v)| k.as_str() != active_key && v.as_str() == model_id.as_str());
            if is_other_provider_model {
                return CommandResult::error(format!(
                    "Invalid model '{name}'. Expected auto or a model for the active provider. Common DeepSeek models: {}",
                    COMMON_DEEPSEEK_MODELS.join(", ")
                ));
...
fn provider_link_info(provider_id: &str) -> ProviderLinkInfo {
    match provider_id {
        ...
        _ => ProviderLinkInfo {
            key_url: None,
            docs_url: "https://deepseek.net/en/docs",
...
pub fn deepseek_links(app: &mut App) -> CommandResult {
    ...
    for provider in deepseek_config::provider::providers_sorted_for_display() {
...
mod tests {
    ...
    fn test_model_change_rejects_invalid_model() {
        let mut app = create_test_app();
        let result = model(&mut app, Some("gpt 4")); // space makes it syntactically invalid
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
    ...
    fn test_deepseek_links() {
        ...
        assert!(!msg.contains("https://deepseek.dev/docs/providers"));
        assert!(result.action.is_none());
    }

    #[test]
    fn provider_link_fallback_uses_current_deepseek_docs() {
        let links = provider_link_info("unknown-provider");

        assert_eq!(links.docs_url, "https://deepseek.net/en/docs");
