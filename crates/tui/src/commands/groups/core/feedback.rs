...
const SECURITY_POLICY_URL: &str = "https://github.com/coohu/DeepSeek-TUI/security/policy";
...
impl FeedbackKind {
    ...
    fn issue_url_base(self) -> &'static str {
        match self {
            Self::Bug => "https://github.com/coohu/DeepSeek-TUI/issues/new?template=bug_report.md",
            Self::Feature => {
                "https://github.com/coohu/DeepSeek-TUI/issues/new?template=feature_request.md"
            }
...
fn bug_report_diagnostics_hint() -> &'static str {
    "Before filing, first check whether this looks like a model issue or an environment/tool issue: \
     ...
     If you have a local JSONL log, run `deepseek session-diagnostics <path>` and include the redacted category summary. \
     Include the DeepSeek version, OS/terminal, the tool name, and redacted timestamps or log handles when available. \
...
mod tests {
    ...
    fn feedback_bug_opens_bug_template_url_without_prefilled_body() {
        ...
        assert!(message.contains("deepseek session-diagnostics <path>"));
    ...
    fn feedback_urls_use_template_only() {
        ...
        assert_eq!(
            bug,
            "https://github.com/coohu/DeepSeek-TUI/issues/new?template=bug_report.md"
        );
        assert_eq!(
            feature,
            "https://github.com/coohu/DeepSeek-TUI/issues/new?template=feature_request.md"
