...
fn discover_visible_skills(app: &App) -> SkillRegistry {
    crate::skills::discover_for_workspace_and_dir_with_mode(
        &app.workspace,
        &app.skills_dir,
        crate::skills::SkillDiscoveryMode::from_deepseek_only(app.skills_scan_deepseek_only),
    )
}

#[cfg(test)]
fn discover_visible_skills(app: &App) -> SkillRegistry {
    let mode = crate::skills::SkillDiscoveryMode::from_deepseek_only(app.skills_scan_deepseek_only);
...
fn skill_discovery_mode_label(mode: crate::skills::SkillDiscoveryMode) -> &'static str {
    match mode {
        ...
        crate::skills::SkillDiscoveryMode::CodeWhaleOnly => "deepseek-only",
    }
}
...
/// local cache (`~/.deepseek/cache/skills/`). Pass `inspect` to show local
...
mod tests {
    ...
    #[test]
    fn test_list_skills_respects_deepseek_only_scan() {
        let tmpdir = TempDir::new().unwrap();
        let _home = IsolatedHome::new(&tmpdir);
        ...
        .unwrap();
        let deepseek_skill_dir = tmpdir
            .path()
            .join(".deepseek")
            .join("skills")
            .join("deepseek-skill");
        std::fs::create_dir_all(&deepseek_skill_dir).unwrap();
        std::fs::write(
        ...
        let mut app = create_test_app_with_tmpdir(&tmpdir);
        app.skills_dir = tmpdir.path().join(".deepseek").join("skills");
        app.skills_scan_deepseek_only = true;
        let result = list_skills(&mut app, None);
        let msg = result.message.unwrap();

        assert!(msg.contains("/deepseek-skill"), "got: {msg}");
        assert!(!msg.contains("/claude-skill"), "got: {msg}");
    }

    #[test]
    fn test_skills_inspect_reports_deepseek_only_scan_mode() {
        let tmpdir = TempDir::new().unwrap();
        let _home = IsolatedHome::new(&tmpdir);
        let claude_skill_dir = tmpdir
            .path()
        ...
        let normalized = msg.replace('\\', "/");
        assert!(
            normalized.contains("Discovery mode: deepseek-only"),
            "got: {msg}"
        );
        assert!(normalized.contains("deepseek-skill"), "got: {msg}");
