...
use crate::config::{
    ApiProvider, Config, DEFAULT_STREAM_CHUNK_TIMEOUT_SECS,
...
fn configured_ask_rules(app: &App) -> CommandResult {
    let store = match deepseek_config::ConfigStore::load(app.config_path.clone()) {
        Ok(store) => store,
...
/// `deepseek auth clear --provider <id>` and
/// `deepseek auth set --provider <id>`.
        ...
        Ok(()) => {
            ...
            CommandResult::message(format!(
                "Cleared API key for {provider_name}. \
                 Use `deepseek auth clear --provider <id>` to clear a different provider."
            ))
        }
...
mod tests {
    ...
    fn config_command_ask_rules_reports_missing_permissions_file() {
        ...
        let permissions_path =
            deepseek_config::resolve_permissions_path(Some(config_path.clone())).unwrap();
...
"#,
    ...
    fn config_command_ask_rules_output_format_is_stable() {
        let rules = vec![
            deepseek_config::ToolAskRule::exec_shell("cargo test"),
            deepseek_config::ToolAskRule::file_path("edit_file", r"src\a.rs"),
        ];
...
2 | edit_file | (any) | src\\a.rs"
    ...
    fn config_reasoning_effort_uses_codex_provider_labels() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-tui-codex-effort-config-test-{}",
            std::process::id()
        ));
    ...
    fn config_fancy_animations_obeys_ghostty_override() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-tui-ghostty-fancy-config-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_command_allow_shell_save_persists_root_boolean() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-allow-shell-save-app-path-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
    ...
    fn config_command_subagents_off_save_persists_and_updates_runtime() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-subagents-off-save-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let config_path = temp_root.join("custom-config.toml");
    ...
    fn config_command_subagents_depth_save_clamps_to_ceiling() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-subagents-depth-save-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let config_path = temp_root.join("custom-config.toml");
    ...
    fn config_command_subagents_status_shows_raw_and_resolved_values() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-subagents-status-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let config_path = temp_root.join("custom-config.toml");
...
"#,
    ...
    fn config_command_audit_lists_editability_and_current_values() {
        let temp_root =
            env::temp_dir().join(format!("deepseek-config-audit-test-{}", std::process::id()));
        fs::create_dir_all(&temp_root).unwrap();
        let config_path = temp_root.join("custom-config.toml");
        fs::write(
            &config_path,
...
"#,
    ...
    fn config_command_stream_chunk_timeout_save_persists_tui_key() {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let temp_root = env::temp_dir().join(format!(
            "deepseek-tui-stream-timeout-test-{}-{}",
            std::process::id(),
            nanos
        ));
        fs::create_dir_all(&temp_root).unwrap();
    ...
    fn config_command_provider_url_token_plan_persists_provider_base_url() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-provider-url-save-app-path-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
    ...
    fn config_approval_mode_save_persists_top_level_policy() {
        let temp_root = env::temp_dir().join(format!(
            "deepseek-approval-policy-save-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let config_path = temp_root.join("custom-config.toml");
