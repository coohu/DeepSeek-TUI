...
#[then("DeepSeek should report that the session was loaded")]
fn deepseek_reports_session_loaded(world: &mut SessionCommandWorld) {
    let message = world
        .last_message
        .as_deref()
...
fn deepseek_rejects_empty_fork(world: &mut SessionCommandWorld) {
    assert_eq!(
...
fn deepseek_reports_one_session_pruned(world: &mut SessionCommandWorld) {
    let message = world
        .last_message
        .as_deref()
...
fn deepseek_triggers_context_compaction(world: &mut SessionCommandWorld) {
    assert_eq!(
        world.last_result_is_error,
        Some(false),
...
fn deepseek_triggers_context_purge(world: &mut SessionCommandWorld) {
    assert_eq!(
        world.last_result_is_error,
        Some(false),
...
#[then(regex = r#"^DeepSeek should send a session relay instruction focused on "([^"]+)"$"#)]
fn deepseek_sends_session_relay_instruction_focused_on(
...
fn deepseek_rejects_unknown_session_command(world: &mut SessionCommandWorld) {
    assert_eq!(
        world.last_result_is_error,
        Some(true),
...
fn execute_isolated(world: &mut SessionCommandWorld, command: &str) -> CommandResult {
    ...
    let _deepseek_home = EnvVarGuard::set("DEEPSEEK_HOME", home.join(".deepseek"));
