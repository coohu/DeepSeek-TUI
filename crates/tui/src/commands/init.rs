...
/// If `workspace` is inside a git repository, ensure workspace-local DeepSeek
...
/// keeping the authored `.deepseek/constitution.json` repo authority policy
/// committable (a directory exclude cannot be overridden, so `.deepseek/*` plus
...
mod tests {
    ...
    fn ensure_deepseek_gitignored_creates_gitignore() {
        ...
        // .deepseek/ is ignored at any depth, but the committed
        // constitution.json is kept.
        assert!(content.contains("**/.deepseek/*"));
        assert!(content.contains("!**/.deepseek/constitution.json"));
    }
    ...
    fn ensure_deepseek_gitignored_idempotent() {
        ...
        ensure_deepseek_gitignored(tmpdir.path());
        let content = std::fs::read_to_string(tmpdir.path().join(".gitignore")).unwrap();
        assert_eq!(content.matches(".deepseek/").count(), 3);
    }
