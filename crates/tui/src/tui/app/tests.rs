...
fn app_new_detects_missing_api_key_with_default_config() {
    ...
    let config_path = tmp.path().join("config.toml");
    let _config_path = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", &config_path);
    let _provider_env = EnvVarGuard::remove("DEEPSEEK_PROVIDER");
...
#[test]
fn cached_skills_respect_deepseek_only_scan_config() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    ...
    let deepseek_dir = workspace
        .join(".deepseek")
        .join("skills")
        .join("deepseek-skill");
    std::fs::create_dir_all(&deepseek_dir).expect("deepseek skill dir");
    std::fs::write(
        deepseek_dir.join("SKILL.md"),
        "---\nname: deepseek-skill\ndescription: DeepSeek skill\n---\nbody\n",
    )
    .expect("write deepseek skill");
    ...
    assert_eq!(app.skills_dir, workspace.join(".deepseek").join("skills"));
    assert!(
        app.cached_skills
            .iter()
            .any(|(name, _)| name == "deepseek-skill"),
        "DeepSeek skill should be cached: {:?}",
        app.cached_skills
    );
    assert!(
        !app.cached_skills
...
#[test]
fn resolve_skills_dir_requires_deepseek_skills_to_be_directory() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    std::fs::create_dir_all(workspace.join(".deepseek")).expect("deepseek dir");
    std::fs::write(
...
#[test]
fn cached_skills_preserve_configured_directory_in_deepseek_only_scan() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");

    let deepseek_skill_dir = workspace
        .join(".deepseek")
        .join("skills")
        .join("workspace-deepseek");
    std::fs::create_dir_all(&deepseek_skill_dir).expect("workspace deepseek skill dir");
    std::fs::write(
        deepseek_skill_dir.join("SKILL.md"),
        "---\nname: workspace-deepseek\ndescription: Workspace DeepSeek skill\n---\nbody\n",
    )
    .expect("write workspace deepseek skill");
    ...
    assert!(
        app.cached_skills
            .iter()
            .any(|(name, _)| name == "workspace-deepseek"),
        "workspace DeepSeek skill should still be cached: {:?}",
        app.cached_skills
    );
    assert!(
        app.cached_skills
...
#[test]
fn cached_skills_reject_deepseek_only_workspace_symlink_escape() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    ...
    let link_path = workspace.join(".deepseek").join("skills");
    ...
    assert!(
        ...
        "strict app cache must not follow escaped workspace DeepSeek symlinks: {:?}",
        app.cached_skills
    );
}
...
fn app_with_fallback_chain(
    active: ApiProvider,
    fallbacks: &[deepseek_config::ProviderKind],
    keyed: &[ApiProvider],
) -> App {
...
fn advance_fallback_skips_unauthed_middle_provider_and_lands_on_next_ready() {
    ...
    let mut app = app_with_fallback_chain(
        ...
        &[
            ...
            deepseek_config::ProviderKind::Together,
        ],
...
fn advance_fallback_local_provider_is_eligible_without_a_key() {
    ...
    let mut app = app_with_fallback_chain(
        ApiProvider::Openai,
        &[deepseek_config::ProviderKind::Ollama],
        &[ApiProvider::Openai],
    );
...
fn advance_fallback_local_primary_may_fall_back_to_local_sibling() {
    ...
    let mut app = app_with_fallback_chain(
        ApiProvider::Ollama,
        &[deepseek_config::ProviderKind::Vllm],
        &[],
    );
