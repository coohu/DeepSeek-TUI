...
const REQUIRED_RELEASE_ASSETS: &[&str] = &[
    ...
    "deepseek-windows-x64-portable.zip",
...
impl TerminalInputPump {
    ...
    fn spawn_parts() -> io::Result<TerminalInputPumpParts> {
        ...
        let handle = thread::Builder::new()
            .name("deepseek-terminal-input".to_string())
            .spawn(move || {
...
pub async fn run_tui(config: &Config, options: TuiOptions) -> Result<()> {
    ...
    if use_alt_screen {
        ...
        // Windows also suppresses DeepSeek's own verbose CLI logger while
...
fn subagent_completion_status(result: &str) -> Option<String> {
    const START: &str = "<deepseek:subagent.done>";
    const END: &str = "</deepseek:subagent.done>";
...
fn build_engine_config(app: &App, config: &Config) -> EngineConfig {
    ...
    EngineConfig {
        ...
        skills_scan_deepseek_only: app.skills_scan_deepseek_only,
                    ...
                    >= TERMINAL_INPUT_RECOVERY_COOLDOWN
            {
                ...
                match terminal_input.restart_detached() {
                    ...
                    Err(err) => {
                        ...
                        app.push_status_toast(
                            "Terminal input stalled; recovery failed. Restart DeepSeek if keys stop responding.",
...
fn recover_engine_event_disconnect(app: &mut App) -> bool {
    ...
    app.add_message(HistoryCell::Error {
        message: "Engine stopped before completing the turn. Check ~/.deepseek/crashes and retry."
            .to_string(),
...
async fn apply_model_and_compaction_update(
    ...
    mode: AppMode,
    route_limits: Option<deepseek_config::route::RouteLimits>,
) {
    let _ = engine_handle
        .send(Op::SetModel {
...
/// `~/.deepseek/settings.toml` (legacy: `~/.deepseek/settings.toml`) so it survives a restart, push the change to
...
async fn switch_provider(
    ...
    *config = next_config;
    ...
    app.provider_chain = target
        .kind()
        .map(|kind| deepseek_config::ProviderChain::new(kind, &config.fallback_providers))
...
fn apply_workspace_runtime_state(app: &mut App, config: &Config, workspace: PathBuf) {
    ...
    app.skills_scan_deepseek_only = config.skills_config().scan_deepseek_only();
...
fn render(f: &mut Frame, app: &mut App, config: &Config) {
    ...
    // Render header
    {
        ...
        let provider_label = match app.api_provider {
            ...
            crate::config::ApiProvider::ShengSuanYun => Some("胜算云"),
...
fn apply_hotbar_setup_saved(
    app: &mut App,
    config: &mut Config,
    bindings: Vec<deepseek_config::HotbarBindingToml>,
) {
...
struct ApprovalDecisionEvent {
    ...
    persistent_ask_rules: Vec<deepseek_config::ToolAskRule>,
}
...
fn persist_ask_rules_from_approval(
    app: &mut App,
    config: &mut Config,
    rules: &[deepseek_config::ToolAskRule],
) {
    match deepseek_config::ConfigStore::load(app.config_path.clone()).and_then(|mut store| {
...
async fn version_hint_from_startup_source(
    ...
    match source {
        ...
        StartupVersionCheckSource::ReleaseResolver => {
            ...
            let body = deepseek_release::fetch_release_json_async(
                deepseek_release::LATEST_RELEASE_URL,
                "latest release",
            )
            .await
            .ok()?;
...
async fn version_hint_from_release_mirror_env(current: &str) -> Option<String> {
    ...
    let tag = deepseek_release::latest_release_tag_async(deepseek_release::ReleaseChannel::Stable)
        .await
        .ok()?;
...
fn release_mirror_env_configured() -> bool {
    let version = deepseek_release::update_version_from_env()
        .unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
    deepseek_release::release_base_url_from_env(&version).is_some()
}
...
async fn version_hint_from_configured_update_uri(
    ...
    let body =
        deepseek_release::fetch_release_json_async(update_uri, "configured latest release").await?;
