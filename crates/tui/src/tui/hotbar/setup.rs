...
pub struct HotbarSetupView {
    ...
    selected_slot: u8,
    original_bindings: BTreeMap<u8, deepseek_config::HotbarBindingToml>,
    draft_bindings: BTreeMap<u8, deepseek_config::HotbarBindingToml>,
...
impl HotbarSetupView {
    ...
    pub fn new(app: &App, config: &Config) -> Self {
        ...
        let original_bindings = config
            ...
            .map(|binding| {
                (
                    binding.slot,
                    deepseek_config::HotbarBindingToml {
                        slot: binding.slot,
    ...
    #[must_use]
    #[cfg(test)]
    pub fn binding_for_slot(&self, slot: u8) -> Option<&deepseek_config::HotbarBindingToml> {
    ...
    pub fn select_slot(&mut self, slot: u8) -> bool {
        if !(1..=deepseek_config::HOTBAR_SLOT_COUNT).contains(&slot) {
            self.validation_errors = vec![format!(
                "Hotbar slot {slot} is outside 1-{}",
                deepseek_config::HOTBAR_SLOT_COUNT
            )];
            return false;
        }
    ...
    #[must_use]
    pub fn save_bindings(&self) -> Vec<deepseek_config::HotbarBindingToml> {
    ...
    fn move_slot(&mut self, delta: isize) {
        let len = usize::from(deepseek_config::HOTBAR_SLOT_COUNT);
    ...
    fn render_lines(&self) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(""));
        let slots = (1..=deepseek_config::HOTBAR_SLOT_COUNT)
            .map(|slot| {
                let label = self
                    .draft_bindings
                    .get(&slot)
