...
impl Default for HotbarRecommendationOptions {
    fn default() -> Self {
        Self {
            max_total: usize::from(deepseek_config::HOTBAR_SLOT_COUNT),
            max_eligible_per_category: usize::MAX,
            include_required_args: false,
        }
    }
...
pub fn recommended_hotbar_bindings(
    app: &App,
    options: HotbarRecommendationOptions,
) -> Vec<deepseek_config::HotbarBindingToml> {
    recommend_hotbar_actions(app, options)
        .into_iter()
        .take(usize::from(deepseek_config::HOTBAR_SLOT_COUNT))
        .enumerate()
        .map(|(idx, entry)| deepseek_config::HotbarBindingToml {
...
fn default_hotbar_position(action_id: &str) -> Option<usize> {
    deepseek_config::DEFAULT_HOTBAR_ACTIONS
        .iter()
...
impl AppHotbarAction {
    ...
    fn recommendation(&self) -> HotbarRecommendation {
        if deepseek_config::DEFAULT_HOTBAR_ACTIONS.contains(&self.id) {
...
mod tests {
    ...
    fn default_hotbar_actions_have_registered_default_metadata() {
        ...
        for id in deepseek_config::DEFAULT_HOTBAR_ACTIONS {
            let action = registry
                .get(id)
    ...
    fn hotbar_recommendations_limit_eligible_actions_by_category() {
        ...
        for default_id in deepseek_config::DEFAULT_HOTBAR_ACTIONS {
            assert!(
                recommendations
                    .iter()
