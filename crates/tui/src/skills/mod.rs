...
pub enum SkillDiscoveryMode {
    /// Preserve the existing broad compatibility scan across DeepSeek,
    /// agentskills.io, Claude, OpenCode, Cursor, and legacy DeepSeek roots.
    Compatible,
    /// Scan only DeepSeek-owned roots. Callers that also pass an explicit
...
impl SkillDiscoveryMode {
    #[must_use]
    pub fn from_deepseek_only(value: bool) -> Self {
        if value {
            Self::CodeWhaleOnly
        } else {
            Self::Compatible
...
/// 6. `<workspace>/.deepseek/skills` — DeepSeek workspace skills.
/// 7. [`agents_global_skills_dir`] — agentskills.io global.
/// 8. `~/.claude/skills` — Claude-ecosystem global (#902).
/// 9. `~/.deepseek/skills` — DeepSeek global, primary install target.
...
fn skills_directories_with_home_and_mode(
    ...
    let mut candidates = match mode {
        ...
        SkillDiscoveryMode::CodeWhaleOnly => deepseek_workspace_skills_dir(workspace)
            .into_iter()
            .collect(),
    };
...
pub(crate) fn deepseek_workspace_skills_dir(workspace: &Path) -> Option<PathBuf> {
    let skills_dir = workspace.join(".deepseek").join("skills");
...
body";
    ...
    #[test]
    fn deepseek_only_mode_ignores_cross_tool_skill_dirs() {
        let tmpdir = TempDir::new().unwrap();
        let workspace = tmpdir.path().join("workspace");
        let home = tmpdir.path().join("home");
        let configured_dir = home.join(".deepseek").join("skills");
        ...
        assert!(names.contains(&"from-deepseek"));
        assert!(names.contains(&"configured-deepseek"));
        assert!(
            !names.contains(&"from-claude") && !names.contains(&"from-agents"),
            "DeepSeek-only mode must not import cross-tool skills: {names:?}"
        );
    }

    #[test]
    fn deepseek_only_mode_still_honors_explicit_configured_dir() {
        let tmpdir = TempDir::new().unwrap();
        let workspace = tmpdir.path().join("workspace");
        let home = tmpdir.path().join("home");
    ...
    #[test]
    fn deepseek_only_mode_rejects_workspace_deepseek_symlink_escape() {
        let tmpdir = TempDir::new().unwrap();
        let workspace = tmpdir.path().join("workspace");
        let home = tmpdir.path().join("home");
        let escape_target = tmpdir.path().join("escape-target");
        std::fs::create_dir_all(workspace.join(".deepseek")).unwrap();
        ...
        assert!(
            ...
            "DeepSeek-only mode must not follow workspace .deepseek/skills outside the workspace"
        );
    }
