...
/// Default task manager data location (`~/.deepseek/tasks`, or legacy
...
pub fn default_tasks_dir() -> PathBuf {
    ...
    dirs::home_dir()
        ...
        .unwrap_or_else(|| PathBuf::from(".deepseek").join("tasks"))
}

fn default_tasks_dir_for_home(home: &Path) -> PathBuf {
    let primary = home.join(".deepseek").join("tasks");
    if primary.is_dir() {
        return primary;
    }
...
mod tests {
    ...
    #[test]
    fn default_tasks_dir_prefers_existing_deepseek_tasks() {
        let temp_home = tempfile::tempdir().unwrap();
        let home = temp_home.path();
        let primary_tasks = home.join(".deepseek").join("tasks");
        let legacy_tasks = home.join(".deepseek").join("tasks");
    ...
    #[test]
    fn default_tasks_dir_uses_deepseek_tasks_for_new_installs() {
        let temp_home = tempfile::tempdir().unwrap();
        let home = temp_home.path();

        assert_eq!(
            default_tasks_dir_for_home(home),
            home.join(".deepseek").join("tasks")
        );
    }
}
