...
pub async fn execute_js_execution_tool(
    ...
    // inherits DeepSeek's proxy environment, so enabling the flag lets
...
mod tests {
    ...
    async fn execute_js_does_not_inherit_parent_secret_env() {
        ...
        let _secret = EnvVarGuard::set("DEEPSEEK_JS_SECRET_LEAK_TEST", "secret-value");
        let tmp = tempdir().expect("tempdir");
        let result = execute_js_execution_tool(
            &json!({
                "code": "process.stdout.write(process.env.DEEPSEEK_JS_SECRET_LEAK_TEST || 'missing')"
            }),
            tmp.path(),
        )
        .await
