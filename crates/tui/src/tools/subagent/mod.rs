...
const SUBAGENT_WORKTREE_ROOT_DIR: &str = ".deepseek-worktrees";
...
/// gated behind `DEEPSEEK_SUBAGENT_PERF_TRACE=1`. The atomic increments are
...
fn subagent_perf_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        std::env::var("DEEPSEEK_SUBAGENT_PERF_TRACE")
...
struct SpawnRequest {
    ...
    cwd: Option<PathBuf>,
    /// Optional first-class git worktree isolation. When set, DeepSeek
...
///
/// Sourced from [`deepseek_config::DEFAULT_SPAWN_DEPTH`] so standalone
...
pub const DEFAULT_MAX_SPAWN_DEPTH: u32 = deepseek_config::DEFAULT_SPAWN_DEPTH;
...
/// `<deepseek:subagent.done>` sentinel that the model expects in the
...
impl ToolSpec for AgentTool {
    ...
    fn description(&self) -> &'static str {
        concat!(
            ...
            "For several independent targets, call agent separately for each target; DeepSeek runs or queues them under runtime capacity and provider rate-limit backpressure. ",
        ...
        json!({
            ...
            "properties": {
                ...
                "model_strength": {
                    "type": "string",
                    "enum": ["same", "faster"],
                    "description": "Optional child model strength. Use same when the child should be as capable as the current model. Use faster for type=explore, read-only lookup/search, status, or other low-risk tasks that can run on a smaller/faster same-family sibling; DeepSeek maps known families such as DeepSeek V4 Pro to Flash and GLM-5.2 to GLM-5-Turbo. type=explore defaults to faster unless you pass model_strength or model explicitly. No hidden auto-downgrade happens."
                },
                "model": {
                    "type": "string",
                ...
                "worktree_path": {
                    "type": "string",
                    "description": "Optional worktree checkout path. Relative paths are created under the default sibling .deepseek-worktrees directory, not inside the parent checkout."
                },
                "fork_context": {
                    "type": "boolean",
...
fn child_completion_runtime_message(completions: &[SubAgentCompletion]) -> Message {
    let mut text = String::from(
        "<deepseek:runtime_event kind=\"child_subagent_completion\" visibility=\"internal\">\n\
...
and distinguish that from evidence you personally verified.\n",
    ...
    text.push_str("</deepseek:runtime_event>");
...
fn parse_spawn_request(input: &Value) -> Result<SpawnRequest, ToolError> {
    ...
    let max_depth = input
        ...
        .map(|depth| {
            let ceiling = deepseek_config::MAX_SPAWN_DEPTH_CEILING;
            u32::try_from(depth)
                .map_err(|_| {
...
fn validate_existing_child_cwd(
    ...
    let canonical = resolved.canonicalize().map_err(|e| {
        ToolError::invalid_input(format!(
            "Invalid cwd '{}': {e} (path may not exist yet — use worktree=true to let DeepSeek create an isolated checkout)",
