...
pub fn default_automations_dir() -> PathBuf {
    ...
    // (not deepseek_home()'s Ok/Err, which succeeds for the default home too).
    if let Some(home) = std::env::var_os("DEEPSEEK_HOME").filter(|value| !value.is_empty()) {
...
mod tests {
    ...
    #[test]
    fn default_automations_dir_honors_deepseek_home_as_hard_override() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempfile::TempDir::new().unwrap();
        // SAFETY: serialised by lock_test_env.
        unsafe {
            std::env::remove_var("DEEPSEEK_AUTOMATIONS_DIR");
            std::env::set_var("DEEPSEEK_HOME", tmp.path());
        }
        // $DEEPSEEK_HOME IS the home dir (no ".deepseek" appended); the
    ...
    #[test]
    fn default_automations_dir_prefers_deepseek_automations_dir_over_deepseek_home() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempfile::TempDir::new().unwrap();
        // SAFETY: serialised by lock_test_env.
        unsafe {
            std::env::set_var("DEEPSEEK_AUTOMATIONS_DIR", tmp.path());
            std::env::set_var("DEEPSEEK_HOME", "/should/not/be/used");
        }
