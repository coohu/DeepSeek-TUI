...
//!
//! Implements a dedicated Responses API client that maps DeepSeek's internal
//! message/tool types to the Responses wire format and parses streaming SSE
//! events back into DeepSeek's `StreamEvent` / `MessageResponse` types.
//!
...
fn build_responses_body(request: &MessageRequest) -> Value {
    ...
    // "low", and DeepSeek's "auto" falls back to "medium".
...
/// Convert DeepSeek messages to Responses API input items.
...
/// Convert a DeepSeek tool definition to a Responses API function tool.
