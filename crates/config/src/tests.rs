...
fn permissions_toml_deserializes_action_deny_and_allow() {
    ...
    assert_eq!(
        ...
        deepseek_execpolicy::PermissionAction::Deny
    );
    assert_eq!(
        permissions.rules[1].action,
        deepseek_execpolicy::PermissionAction::Allow
    );
    assert_eq!(
        permissions.rules[2].action,
        deepseek_execpolicy::PermissionAction::Ask
    ); // default
}
...
fn provider_command_auth_source_deserializes() {
    let config: ConfigToml = toml::from_str(
        ...
        source = "command"
        command = ["keepassxc-cli", "show", "DeepSeek/DeepSeek", "--attribute", "password"]
        timeout_ms = 2000
        "#,
    )
    .expect("config toml");
...
fn provider_secret_auth_source_deserializes() {
    ...
    assert_eq!(auth.secret_id.as_deref(), Some("deepseek/openai"));
...
///   3. Switch the active provider through `DEEPSEEK_PROVIDER` —
...
fn app_homes_prefer_home_env_before_platform_home_fallback() {
        ...
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                match self.deepseek_home.take() {
                    Some(value) => env::set_var("DEEPSEEK_HOME", value),
                    None => env::remove_var("DEEPSEEK_HOME"),
                }
            }
        }
    }
    ...
    let _env = HomeEnvGuard {
        ...
        deepseek_home: env::var_os("DEEPSEEK_HOME"),
    };
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
    ...
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("DEEPSEEK_HOME", &explicit);
    }
...
fn migrate_config_reports_copied_legacy_path() {
    ...
    unsafe {
        ...
        env::set_var("DEEPSEEK_HOME", &primary_dir);
    }
...
/// `DEEPSEEK_HOME` so `deepseek_home()` -> `<home>/.deepseek` and
...
impl StateDirEnv {
    fn install(unique: u128) -> Self {
        let home = std::env::temp_dir().join(format!(
            "deepseek-state-migration-{}-{unique}",
            std::process::id()
        ));
    ...
    fn primary(&self, sub: &str) -> PathBuf {
        self.home.join(DEEPSEEK_APP_DIR).join(sub)
    }
}
...
fn state_migration_notice_explains_preserved_data_and_canonical_root() {
    let migration = StateMigration {
        ...
        primary_path: PathBuf::from("/home/alice/.deepseek/sessions"),
        kind: StateMigrationKind::Relocated,
    };

    let notice = migration.user_notice();

    assert!(notice.contains("DeepSeek migrated legacy state"));
    assert!(notice.contains("/home/alice/.deepseek/sessions"));
    assert!(notice.contains("/home/alice/.deepseek/sessions"));
    assert!(notice.contains("Your data was preserved"));
    assert!(notice.contains("Use .deepseek as the canonical state location"));
...
fn copied_state_migration_notice_says_legacy_copy_remains() {
    let migration = StateMigration {
        ...
        primary_path: PathBuf::from("/home/alice/.deepseek/catalog"),
...
#[test]
fn explicit_deepseek_home_bypasses_legacy_state_fallback_and_migration() {
    let _lock = env_lock();
    let unique = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .expect("clock")
        .as_nanos();
    let state_env = StateDirEnv::install(unique);
    let explicit_home = state_env.home.join("isolated-deepseek");
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("DEEPSEEK_HOME", &explicit_home);
    }
    fs::create_dir_all(state_env.legacy("catalog")).expect("legacy dir");
    ...
    assert_eq!(
        resolve_state_dir("catalog").expect("resolve"),
        primary,
        "explicit DEEPSEEK_HOME must not read ambient legacy state"
    );
    ...
    assert!(
        ...
        "explicit DEEPSEEK_HOME must not migrate ambient legacy state"
    );
    assert!(
        !primary.join("legacy.json").exists(),
        "legacy contents must not be copied into an explicit DEEPSEEK_HOME"
    );
    let _ = fs::remove_dir_all(&state_env.home);
}
...
fn project_state_resolvers_reject_path_traversal_subdirs() {
    ...
    assert_eq!(
        created,
        canonical_workspace.join(DEEPSEEK_APP_DIR).join("a/b")
    );
}
...
fn resolve_config_path_rejects_env_traversal() {
    ...
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("DEEPSEEK_CONFIG_PATH", "../config.toml");
...
fn load_project_config_rejects_symlinked_primary_config() {
    ...
    let primary_dir = workspace.path().join(DEEPSEEK_APP_DIR);
...
fn config_store_save_skips_identical_serialized_body() {
    ...
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!(
        "deepseek-config-noop-save-{}-{unique}",
        std::process::id()
    ));
    fs::create_dir_all(&dir).expect("mkdir");
    let path = dir.join(CONFIG_FILE_NAME);
...
fn config_store_save_creates_one_time_backup_before_changed_write() {
    ...
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!(
        "deepseek-config-backup-save-{}-{unique}",
        std::process::id()
    ));
    fs::create_dir_all(&dir).expect("mkdir");
    let path = dir.join(CONFIG_FILE_NAME);
...
fn config_store_save_preserves_comments_with_other_keys() {
    ...
    // then changes model via `deepseek config set model`.
    let dir = tempfile::tempdir().expect("tempdir");
    let config_path = dir.path().join(CONFIG_FILE_NAME);
    fs::write(
        &config_path,
...
fn first_party_provider_env_model_overrides_pass_through() {
    ...
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "minimax");
...
fn sakana_env_overrides_resolve_fugu_route() {
    ...
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "sakana");
...
/// `DEEPSEEK_PROVIDER` is the user-facing env alias for switching the
...
/// When both `DEEPSEEK_PROVIDER` and the legacy `DEEPSEEK_PROVIDER`
/// are set, the public alias wins — a user adopting `DEEPSEEK_*` in a
...
/// `DEEPSEEK_MODEL` is the user-facing env alias for picking a model
...
fn deepseek_model_env_alias_overrides_default_for_active_provider() {
    ...
    unsafe {
        env::set_var("DEEPSEEK_PROVIDER", "moonshot");
        env::set_var("DEEPSEEK_MODEL", "custom-kimi-test-model");
    }
    let config = ConfigToml::default();
...
fn test_verbosity_resolution() {
    ...
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    unsafe {
        std::env::set_var("DEEPSEEK_VERBOSITY", "normal");
    }
