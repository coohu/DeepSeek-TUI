...
- Added `deepseek doctor` / `deepseek doctor --json` legacy-state
  diagnostics that compare known `~/.deepseek` state paths with their
  `~/.deepseek` counterparts and flag unmigrated or dual-root data (#3727).
...
- Added a release evidence ledger for v0.8.66 and opened the external ACP
  registry submission for DeepSeek after validating the published
  `deepseek@0.8.65` ACP auth handshake against the upstream registry checker
  (#3192).
...
- Surfaced legacy state relocation with a user-visible migration notice whenever
  `~/.deepseek/<state>` is moved or copied into `~/.deepseek/<state>`, so
...
- Restored legacy `.deepseek/sessions` visibility for upgraded installs where
  an empty `~/.deepseek/sessions` directory already existed, by copying
  missing legacy session entries into the primary DeepSeek session store
  without overwriting newer data (#3724).
- Calmed approval risk classification for read-only shell commands such as
  `deepseek --version`, `deepseek --help`, and `git status --porcelain` so
...
- Fixed Ctrl+O external-editor freezes where DeepSeek's terminal input pump
...
- Updated the `/links` provider fallback to the current DeepSeek docs URL and
  added a Baidu Qianfan docs link. Harvested from #3621 by @noaft.
- Hardened `DEEPSEEK_TOOL_SURFACE=shell-only` for benchmark/exec runs: the
  shell-only surface hides native tools from the model-visible catalog, and
  unknown `DEEPSEEK_TOOL_SURFACE` values now warn instead of silently falling
...
- **Website and automation.** A runtime/integrations page, provenance and
  ...
  weekly community digest archive on deepseek.net (#3419, #3421, #3415, #3482,
...
- **Legacy app-server non-loopback auth hardening (#3258).** Bare
  `deepseek app-server --host 0.0.0.0` now fails fast unless an explicit
  `--auth-token` or `DEEPSEEK_APP_SERVER_TOKEN` is supplied, keeping generated
...
- **Legacy `.deepseek` state write-path migration (#3240).** State subdirectories
  ...
  `~/.deepseek/`, and the first write of a subdir relocates any pre-existing
...
- **State subdir validation on Windows (#3240).** State path hardening now
  ...
  migrating state directories, keeping the `.deepseek` write resolver inside
...
- **GLM-5.2 is now the default direct Z.AI model.** `DEFAULT_ZAI_MODEL` resolves
  to `GLM-5.2` in both `deepseek-tui` and `deepseek-config`; the `glm-5.1`
...
- **Provider metadata centralization.** Provider env vars, config keys, aliases,
  ...
  across `deepseek-config`, `deepseek-tui`, and `deepseek-cli`, reducing drift
  between the provider picker, `deepseek auth`, `doctor --json`, and setup
  hints.
...
- **Transcript hyperlinks — out-of-band OSC 8 (#3029).** Clickable file /
  ...
  defense-in-depth.
- **DeepSeek-only skill discovery gate (#3296).** New
  `[skills].scan_deepseek_only = true` limits session-time skill discovery to
  DeepSeek-owned roots (`<workspace>/.deepseek/skills`, `~/.deepseek/skills`,
...
- **Runtime API no-auth documentation.** `docs/RUNTIME_API.md` now documents
  `deepseek app-server --insecure-no-auth` for loopback-only testing and warns
...
- **TUI polish.** The empty-startup welcome block is centered by the actual
  ...
  explain whether DeepSeek can retry before output, or is surfacing a warning
...
- Legacy `deepseek` install/update path resolves to `deepseek`. (#2960, #2924, #2917)
...
- Config robustness: atomic permission-rule save, one-time config `.bak` backup before the first changed write, `DEEPSEEK_HOME` as primary config home, and accepting the dispatcher-written config shape (camelCase aliases + `[features.enabled]` table) so legacy/dual-written configs parse cleanly
...
- **Agent Fleet real-run cutover (#3154/#3096).** `deepseek fleet run` now
  launches durable workers through the headless `deepseek exec --output-format
...
- **Ephemeral generated project context (#3058).** Opening DeepSeek in a
  directory with no instruction files now keeps the bounded generated project
  overview in memory instead of creating `.deepseek/instructions.md`.
...
- **Project-local hook trust boundary (#3140).** `.deepseek/hooks.toml` is now
...
- **SiliconFlow China provider config (#2893/#2895).** `siliconflow-CN`
  ...
  @idling11 for the PR.
- **Self-update download timeout (#3006).** `deepseek update` now applies a
...
- **Legacy `deepseek` update migration (#2960/#3013/#3053).** Running
  ...
  steps instead of trying to spawn a missing `deepseek` binary. README and
...
- **Short `codew` shim delegation.** The `codew` convenience binary now
  prefers the sibling `deepseek` dispatcher installed next to it before
...
- **Exec auto-model handoff (#3148).** `deepseek exec --model auto` now
  survives the CLI/TUI boundary by honoring the DeepSeek model env alias and
...
- **OpenAI Codex context metadata (#3070).** The `gpt-5.5` default and
  DeepSeek aliases now use OpenAI's documented 1,050,000-token context window
...
- **Hooks v2.** `tool_call_before` hooks can now return a JSON decision —
  ...
  project-local `.deepseek/hooks.toml` (#3026).
...
- `deepseek exec` gains `--allowed-tools`, `--disallowed-tools` (deny wins),
...
- Agent-facing polish: the system prompt environment block reports
  `deepseek_version` (was `deepseek_version`), the legacy
...
- **Docker images build again.** The release `docker` job failed for v0.8.56
  ...
  `deepseek-tui` binaries; they are now symlinks to the deepseek binaries
...
- **Volcengine provider in TUI dispatcher.** The `deepseek` / `deepseek-tui`
...
- **Oversized paste spill.** Pastes larger than ~10 KB are now written to
  `.deepseek/pastes/` instead of being truncated or dropped, preserving the
...
- Added provider-scoped `insecure_skip_tls_verify` for private
  ...
  is surfaced by `deepseek doctor`; `SSL_CERT_FILE` remains the preferred path
...
- Added the first `deepseek-whaleflow` foundation crate with typed workflow
...
- Added a static prompt composer override for embedders that need to replace
  ...
  under DeepSeek's runtime prompt assembly. This refines the embedder prompt
...
- Added a metadata-only `deepseek-config` provider registry with canonical
...
- Added optional `[search].base_url` / `DEEPSEEK_SEARCH_BASE_URL` support for
...
- Removed the deprecated `deepseek` and `deepseek-tui` binary shims from the
  ...
  `deepseek`, `codew`, and `deepseek-tui` entry points remain, the private
...
- `/config` now reports the canonical `~/.deepseek/settings.toml` path for TUI
  settings while still reading legacy DeepSeek-branded settings fallbacks and
  migrating them into the DeepSeek home on load.
- Provider switches now roll back transactionally when the first request to a
  newly selected provider fails authentication: DeepSeek restores the previous
...
- Durable task recovery no longer requeues tasks that were `running` when the
  previous DeepSeek process exited. On restart those records are marked failed
...
- Auto-generated project instructions now reuse the bounded Project Context
  ...
  `.deepseek/instructions.md` file exists. The fallback keeps later
...
- The TUI footer, `/status`, `/mcp` manager, and command-palette MCP entries
  now count trusted workspace-local `.deepseek/mcp.json` servers together with
  the global MCP config, matching `deepseek mcp list` for merged global +
...
- `deepseek sessions` now shows `deepseek resume <session-id>` in the footer
  instead of the invalid dispatcher command `deepseek --resume <session-id>`
  (#2758, #2760).
...
(#2636), **@xyuai** for canonical DeepSeek
...
- **Cygwin/MSYS2 config paths honor exported `$HOME`.** DeepSeek and legacy
  DeepSeek config roots now prefer a non-empty `$HOME` before falling back to the
  platform home resolver, while `DEEPSEEK_HOME` remains the strongest explicit
  override (#2369, #2610).
...
- **Arcee AI as a direct provider.** New `[providers.arcee]` config block and
  ...
  wired through CLI auth (`deepseek auth set --provider arcee`), the TUI
...
- Added a Windows NSIS installer release artifact and classroom/lab deployment
  ...
  `DeepSeekSetup.exe` from the canonical Windows binaries, and the installer
...
- Added Xiaomi MiMo speech/TTS support with a `deepseek speech` CLI command,
...
- Added a Cache Guard CI integration test suite simulating prefix-cache
  behaviour across nine scenarios, gated behind `DEEPSEEK_CACHE_GUARD=1`,
  harvested from #2503.
...
- Made legacy config migration visible when DeepSeek copies old DeepSeek-era
  config into the DeepSeek config path.
...
- Fixed the DeepSeek V4-Pro cost estimate after the 2026-05-31 pricing cutoff:
  ...
  so DeepSeek no longer shows roughly 4x too much after June 1 (#2489).
...
- **DeepSeek-first release framing, project-context logging, state-root
  migration, DeepSeek README paths, and reasoning-locale behavior** were
...
- **`DEEPSEEK_*` env aliases.** `DEEPSEEK_PROVIDER`, `DEEPSEEK_MODEL`,
  and `DEEPSEEK_BASE_URL` are public product-scoped aliases that take
...
- **App state migrates to `~/.deepseek/`.** New installs write product-owned
  ...
  no forced migration. `DEEPSEEK_HOME` and `DEEPSEEK_CONFIG_PATH` env vars
  are now supported alongside existing `DEEPSEEK_*` vars (#2011).
- **Project config overlay prefers `.deepseek/config.toml`** before
  `.deepseek/config.toml`. Both are read; the DeepSeek root takes precedence.
...
- **README and web surface rebrand refinements.** Crate descriptions, npm
  package text, and website copy now consistently position DeepSeek as
...
- **DeepSeek now ships with the Brother Whale agent identity prompt.** The
...
- **DeepSeek positioning is clarified as DeepSeek-first and open-model
  oriented.** README, rebrand notes, crate metadata, and npm package text now
  describe DeepSeek as an agentic terminal for open source and open-weight
...
- **Rebrand shim docs now match the v0.8.x transition window.** The npm and
  migration notes no longer imply the legacy `` package/shims
...
- The `deepseek` and `` binary names continue to ship as
...
- **Homebrew tap automation is release-gated.** The release workflow can update
  `coohu/homebrew-` from the checksum manifest when a tap token is
...
- **Companion binary version smoke is unambiguous.** ` --version`
  now reports the `` binary name instead of the dispatcher label.
...
- **Snapshots no longer try to index a multi-hundred-GB workspace
  on first turn.** Reported by users running `` inside
...
- **npm wrapper installs cleanly on OpenHarmony / HarmonyPC**
  ...
  `linux` / `darwin` / `win32`, so `npm i -g ` would
...
- **`deepseek update` now refreshes the companion TUI binary
  ...
  **@NorethSea**). Closes the documented two-binary footgun:
  ...
  but `~/.cargo/bin/` would stay at the previously
...
release tags reliably reach `cnb.cool/.com/DeepSeek-TUI`
...
- **CNB mirror workflow** (PR #1373 from **@Anyexyz**) — a
  ...
  the `cnb.cool/.com/DeepSeek-TUI` repository,
...
- **HTTP User-Agent header** (#1320) — all outbound API requests now
  carry `/{version}` in the User-Agent, matching the format
...
- **Prebuilt Docker images on GHCR** - release builds now publish
  `ghcr.io/hmbown/` with `latest`, semver, and `vX.Y.Z` tags, and
...
- **PTY/frame-capture TUI QA harness** — new
  ...
  `` in a real pseudo-terminal, send scripted keys / paste /
...
- **Windows Scoop install instructions** (#696) — README and zh-CN README now
  document `scoop install ` for Windows users. *Thanks to
...
- **`cargo install` on stable Rust** — the language-picker match guard at
  ...
  ` failed with E0658. Rewrote as a plain match guard with a
...
- **Linux ARM64 prebuilt binaries** — the release workflow now publishes
  `deepseek-linux-arm64` and `-linux-arm64` (built natively on
...
- README and `README.zh-CN.md` now have an explicit **Linux ARM64** quickstart
  pointing ARM64 users at `cargo install -cli  --locked`
  for v0.8.7 and at `npm i -g ` for v0.8.8+.
...
- **Missing-companion error was hostile to direct GitHub Release downloaders**
  ...
  text with a concrete three-path checklist: `npm install -g `,
  `cargo install -cli  --locked`, or downloading both
  `deepseek-<platform>` AND `-<platform>` from the same Release
...
- **Windows release build (LNK1104)** — drop the `deepseek` shim binary in
  ...
  `-cli` artifact during workspace builds; the second linker
...
- **`cargo install` UX** — to install the canonical `deepseek` command,
  `cargo install -cli` (the historical path). The 0.8.1
  one-command flow (`cargo install ` providing both binaries) is
...
### Fixed
- **One-command Cargo install** — `cargo install  --locked` now
  provides both the canonical `deepseek` dispatcher and the ``
  companion binary from the main `` package, so dispatcher
...
- **Windows .exe sibling lookup** — `locate_sibling_tui_binary` in the CLI dispatcher finds `.exe` on Windows, honours `DEEPSEEK_TUI_BIN` override, and falls back to suffix-less lookup. Tests lock in platform-correct name resolution and env override. (#247)
...
### Fixed
- **Windows npm install path** — the npm-distributed `deepseek` dispatcher now locates the platform-correct `` binary (`.exe` suffix on Windows), fixing runtime failures for Windows users. (#247)
...
- Stripped phantom `web.run` references from prompts and the `web_search` tool surface ([#25](https://github.com/coohu/DeepSeek-TUI/issues/25)).
...
- npm wrapper first-run downloads now use process-unique temp files so concurrent `deepseek` / `` invocations do not race on `*.download` files.
...
### Added
- Added npm publish-time release asset verification for the `` package to fail fast when expected GitHub binaries are missing.
- Added checksum manifests to GitHub release assets and checksum verification in the npm installer.
- Added `npm pack` install-and-smoke CI coverage for the `` wrapper package.
...
- Moved the `` binary target inside `crates/tui` so `cargo publish --dry-run -p ` works from the workspace package layout.
...
- Updated release workflow to build both `deepseek` and `` binaries.
...
[0.8.37]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.36...v0.8.37
[0.8.36]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.35...v0.8.36
[0.8.35]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.34...v0.8.35
[0.8.34]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.33...v0.8.34
[0.8.33]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.32...v0.8.33
[0.8.32]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.31...v0.8.32
[0.8.31]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.30...v0.8.31
[0.8.30]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.29...v0.8.30
[0.8.29]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.28...v0.8.29
[0.8.28]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.27...v0.8.28
[0.8.27]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.26...v0.8.27
[0.8.26]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.25...v0.8.26
[0.8.25]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.24...v0.8.25
[0.8.24]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.23...v0.8.24
[0.8.23]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.22...v0.8.23
[0.8.22]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.21...v0.8.22
[0.8.21]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.20...v0.8.21
[0.8.20]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.19...v0.8.20
[0.8.19]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.18...v0.8.19
[0.8.18]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.17...v0.8.18
[0.8.17]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.16...v0.8.17
[0.8.16]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.15...v0.8.16
[0.8.15]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.13...v0.8.15
[0.8.13]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.12...v0.8.13
[0.8.12]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.11...v0.8.12
[0.8.11]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.10...v0.8.11
[0.8.10]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.8...v0.8.10
[0.8.8]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.7...v0.8.8
[0.8.7]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.6...v0.8.7
[0.8.6]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.5...v0.8.6
[0.8.5]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.4...v0.8.5
[0.8.4]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.3...v0.8.4
[0.8.3]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.2...v0.8.3
[0.8.2]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.1...v0.8.2
[0.8.1]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/coohu/DeepSeek-TUI/compare/v0.7.9...v0.8.0
[0.7.9]: https://github.com/coohu/DeepSeek-TUI/compare/v0.7.8...v0.7.9
[0.7.8]: https://github.com/coohu/DeepSeek-TUI/compare/v0.7.7...v0.7.8
[0.7.7]: https://github.com/coohu/DeepSeek-TUI/compare/v0.7.6...v0.7.7
[0.7.6]: https://github.com/coohu/DeepSeek-TUI/compare/v0.7.5...v0.7.6
[0.6.1]: https://github.com/coohu/DeepSeek-TUI/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/coohu/DeepSeek-TUI/compare/v0.4.9...v0.6.0
[0.4.9]: https://github.com/coohu/DeepSeek-TUI/compare/v0.4.8...v0.4.9
[0.4.8]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.33...v0.4.8
[0.3.33]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.32...v0.3.33
[0.3.32]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.31...v0.3.32
[0.3.31]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.28...v0.3.31
[0.3.28]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.27...v0.3.28
[0.3.23]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.22...v0.3.23
[0.3.22]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.21...v0.3.22
[0.3.21]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.17...v0.3.21
[0.3.17]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.16...v0.3.17
[0.3.16]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.14...v0.3.16
[0.3.14]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.13...v0.3.14
[0.3.13]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.12...v0.3.13
[0.3.12]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.11...v0.3.12
[0.3.11]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.10...v0.3.11
[0.3.10]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.6...v0.3.10
[0.3.6]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.5...v0.3.6
[0.3.5]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.4...v0.3.5
[0.3.4]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.3...v0.3.4
[0.3.3]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.2...v0.3.3
[0.3.2]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.1...v0.3.2
[0.3.1]: https://github.com/coohu/DeepSeek-TUI/compare/v0.3.0...v0.3.1
[0.3.0]: https://github.com/coohu/DeepSeek-TUI/compare/v0.2.2...v0.3.0
[0.2.2]: https://github.com/coohu/DeepSeek-TUI/compare/v0.2.0...v0.2.2
[0.2.0]: https://github.com/coohu/DeepSeek-TUI/releases/tag/v0.2.0
[0.0.2]: https://github.com/coohu/DeepSeek-TUI/releases/tag/v0.0.2
[0.0.1]: https://github.com/coohu/DeepSeek-TUI/releases/tag/v0.0.1
[0.1.9]: https://github.com/coohu/DeepSeek-TUI/compare/v0.1.8...v0.1.9
[0.1.8]: https://github.com/coohu/DeepSeek-TUI/compare/v0.1.7...v0.1.8
[0.1.7]: https://github.com/coohu/DeepSeek-TUI/compare/v0.1.6...v0.1.7
...
=======
[Unreleased]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.40...HEAD
[0.8.40]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.39...v0.8.40
[0.8.39]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.38...v0.8.39
[0.8.38]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.37...v0.8.38
[0.8.37]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.36...v0.8.37
[0.8.36]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.35...v0.8.36
[0.8.35]: https://github.com/coohu/DeepSeek-TUI/compare/v0.8.34...v0.8.35
...
=======
[Unreleased]: https://github.com/coohu/deepseek-tui/compare/v0.8.46...HEAD
=======
[Unreleased]: https://github.com/coohu/deepseek-tui/compare/v0.8.54...HEAD
[0.8.54]: https://github.com/coohu/deepseek-tui/compare/v0.8.53...v0.8.54
[0.8.53]: https://github.com/coohu/deepseek-tui/compare/v0.8.52...v0.8.53
[0.8.52]: https://github.com/coohu/deepseek-tui/compare/v0.8.51...v0.8.52
[0.8.51]: https://github.com/coohu/deepseek-tui/compare/v0.8.50...v0.8.51
[0.8.50]: https://github.com/coohu/deepseek-tui/compare/v0.8.49...v0.8.50
[0.8.49]: https://github.com/coohu/deepseek-tui/compare/v0.8.48...v0.8.49
[0.8.48]: https://github.com/coohu/deepseek-tui/compare/v0.8.47...v0.8.48
[0.8.47]: https://github.com/coohu/deepseek-tui/compare/v0.8.46...v0.8.47
>>>>>>> 9463266cb1278cdb82ada338479f32d47f31a704
[0.8.46]: https://github.com/coohu/deepseek-tui/compare/v0.8.45...v0.8.46
[0.8.45]: https://github.com/coohu/deepseek-tui/compare/v0.8.44...v0.8.45
[0.8.44]: https://github.com/coohu/deepseek-tui/compare/v0.8.43...v0.8.44
[0.8.43]: https://github.com/coohu/deepseek-tui/compare/v0.8.42...v0.8.43
[0.8.42]: https://github.com/coohu/deepseek-tui/compare/v0.8.41...v0.8.42
[0.8.41]: https://github.com/coohu/deepseek-tui/compare/v0.8.40...v0.8.41
[0.8.40]: https://github.com/coohu/deepseek-tui/compare/v0.8.39...v0.8.40
[0.8.39]: https://github.com/coohu/deepseek-tui/compare/v0.8.38...v0.8.39
[0.8.38]: https://github.com/coohu/deepseek-tui/compare/v0.8.37...v0.8.38
[0.8.37]: https://github.com/coohu/deepseek-tui/compare/v0.8.36...v0.8.37
...
[Unreleased]: https://github.com/coohu/deepseek-tui/compare/v0.8.64...HEAD
[0.8.65]: https://github.com/coohu/deepseek-tui/compare/v0.8.64...v0.8.65
[0.8.64]: https://github.com/coohu/deepseek-tui/compare/v0.8.63...v0.8.64
[0.8.63]: https://github.com/coohu/deepseek-tui/compare/v0.8.62...v0.8.63
[0.8.62]: https://github.com/coohu/deepseek-tui/compare/v0.8.61...v0.8.62
[0.8.61]: https://github.com/coohu/deepseek-tui/compare/v0.8.60...v0.8.61
[0.8.60]: https://github.com/coohu/deepseek-tui/compare/v0.8.59...v0.8.60
[0.8.59]: https://github.com/coohu/deepseek-tui/compare/v0.8.58...v0.8.59
[0.8.58]: https://github.com/coohu/deepseek-tui/compare/v0.8.57...v0.8.58
[0.8.57]: https://github.com/coohu/deepseek-tui/compare/v0.8.56...v0.8.57
[0.8.56]: https://github.com/coohu/deepseek-tui/compare/v0.8.55...v0.8.56
[0.8.55]: https://github.com/coohu/deepseek-tui/compare/v0.8.54...v0.8.55
[0.8.54]: https://github.com/coohu/deepseek-tui/compare/v0.8.53...v0.8.54
[0.8.53]: https://github.com/coohu/deepseek-tui/compare/v0.8.52...v0.8.53
[0.8.52]: https://github.com/coohu/deepseek-tui/compare/v0.8.51...v0.8.52
[0.8.51]: https://github.com/coohu/deepseek-tui/compare/v0.8.50...v0.8.51
