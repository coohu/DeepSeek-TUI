...
> **このターミナルネイティブのコーディングエージェントは、DeepSeek V4 の 100 万トークンのコンテキストウィンドウとプレフィックスキャッシュ機能を中心に構築されています。`deepseek` ディスパッチャーと `deepseek-tui` ランタイムの Rust バイナリペアとして配布され、Node.js や Python のランタイムは不要です。MCP クライアント、サンドボックス、永続的なタスクキューも標準で同梱されています。**
...
`deepseek` は自己完結型の Rust リリースバイナリのペアとしてインストールされます。`deepseek` はディスパッチャーで、同じ場所にある `deepseek-tui` ランタイムを起動して対話セッションを実行します。npm、Homebrew、Docker は両方を自動でインストールします。Cargo や手動インストールでは、両方を同じディレクトリ（通常は `PATH` 上のディレクトリ）に置いてください。実行に Node.js や Python のランタイムは不要です。
...
cargo install deepseek-cli --locked   # `deepseek` (エントリーポイント)
cargo install deepseek-tui     --locked   # `deepseek-tui` (TUI バイナリ)

# 3. Homebrew — macOS パッケージマネージャ。
#    tap/formula 名は旧名のままですが、deepseek と deepseek-tui をインストールします。
brew tap coohu/
brew install 

# 4. 直接ダウンロード — GitHub Releases のプラットフォームアーカイブ。
#    https://github.com/Hmbown/CodeWhale/releases
#    アーカイブには deepseek と deepseek-tui とインストールスクリプトが含まれます。
...
[![npm](https://img.shields.io/npm/v/deepseek)](https://www.npmjs.com/package/deepseek)
...
![deepseek スクリーンショット](assets/screenshot.png)
...
オープンソース、ターミナルネイティブ、`deepseek` / `deepseek-tui` の Rust バイナリペアとして提供されています。
...
初回起動時に [DeepSeek API キー](https://platform.deepseek.com/api_keys) の入力を求められます。キーは `~/.deepseek/config.toml`（旧 `~/.deepseek/config.toml` も互換性維持）に保存されるため、OS のクレデンシャルプロンプトなしに任意のディレクトリから利用できます。
...
`npm i -g ` は v0.8.8 以降、glibc ベースの ARM64 Linux で動作します。[Releases ページ](https://github.com/coohu/DeepSeek-TUI/releases) からビルド済みバイナリをダウンロードし、`PATH` 上に並べて配置することもできます。
...
```bash
cargo install deepseek-cli --locked   # `deepseek` を提供
cargo install deepseek-tui     --locked   # `deepseek-tui` を提供
deepseek --version
```

ビルド済みバイナリは [GitHub Releases](https://github.com/coohu/DeepSeek-TUI/releases) からもダウンロードできます。ミラーされたリリースアセットには `DEEPSEEK_TUI_RELEASE_BASE_URL` を使ってください。

### Windows（Scoop）

[Scoop](https://scoop.sh) は Windows のパッケージマネージャです。`deepseek`
パッケージは Scoop main bucket にありますが、manifest は GitHub/npm/Cargo
リリースより遅れることがあります。先に更新し、インストール後に
`deepseek --version` で確認してください:
...
ユーザー設定: `~/.deepseek/config.toml`（旧 `~/.deepseek/config.toml` も互換性維持）。プロジェクトオーバーレイ: `<workspace>/.deepseek/config.toml`（旧 `<workspace>/.deepseek/config.toml`）（拒否される項目: `api_key`、`base_url`、`provider`、`mcp_config_path`）。すべてのオプションは [config.example.toml](config.example.toml) にあります。
...
deepseek はワークスペースのディレクトリ（`.agents/skills` → `skills` → `.opencode/skills` → `.claude/skills`）とグローバルな `~/.deepseek/skills`（旧 `~/.deepseek/skills` も互換性維持）からスキルを発見します。各スキルは `SKILL.md` ファイルを持つディレクトリです:
...
[CONTRIBUTING.md](CONTRIBUTING.md) を参照してください。プルリクエストを歓迎します。良い初コントリビューションは [Open Issues](https://github.com/coohu/DeepSeek-TUI/issues) を確認してください。
