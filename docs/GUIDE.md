...
This guide is for your first hour with DeepSeek. It explains the main
workflow, the important safety controls, and where to go next when you need a
complete reference.

DeepSeek has deeper reference documents for installation, configuration,
...
## 1. Welcome to DeepSeek

DeepSeek is a terminal coding agent. You run it from a workspace, give it a
task, and it can use structured tools to inspect files, run commands, edit
code, and report back with evidence.

The important difference from a normal chat model is that DeepSeek is built
around a harness:
...
You can use DeepSeek for small questions:
...
For a new repository, start conservatively. Ask DeepSeek to explore and plan
...
Install DeepSeek with the path that fits your machine. Each supported install
path provides both the `deepseek` dispatcher and the `deepseek-tui` runtime.
...
Launch DeepSeek from the repository or directory you want it to work in:

```bash
deepseek
```

On first launch, DeepSeek needs an API key for the active provider. DeepSeek is
...
New DeepSeek config is stored under `~/.deepseek/config.toml`. Legacy
...
```text
Investigate why `deepseek doctor` reports the wrong provider. Do not edit
files yet. Return the likely cause, evidence, and a proposed patch plan.
```

DeepSeek works best when you let investigation and implementation happen in
...
The transcript is the audit trail. When DeepSeek reads files, runs commands,
...
DeepSeek has three visible TUI modes:
...
Agent mode is the default for most contribution work. It lets DeepSeek read,
...
change DeepSeek state directly instead of asking the model in natural
language.
...
Use `/model auto` when you want DeepSeek to choose the model and thinking
...
DeepSeek tools are structured actions. Instead of only producing prose, the
...
The workspace boundary matters. DeepSeek is expected to work in the directory
...
that teaches DeepSeek how to perform a recurring workflow, use a tool family,
...
### Is DeepSeek only for DeepSeek?

DeepSeek is the default and first-class route, but DeepSeek also supports
other hosted and local OpenAI-compatible providers. Use `/provider` or
`deepseek --provider <id>` to choose a provider. Keep the provider registry
...
### Why does DeepSeek ask before running commands?
...
Inside DeepSeek, ask the agent to inspect the file and run it with
...
New DeepSeek config uses `~/.deepseek/config.toml`. Legacy
...
profile, and compact long sessions. For larger tasks, ask DeepSeek to plan
before implementing so you do not spend tokens on the wrong path.

### How do I continue previous work?

DeepSeek saves sessions. Use the session picker or resume/continue CLI paths
...
from every workspace, or run `deepseek sessions` to list all saved sessions
...
problem is operational, run `deepseek doctor` and inspect the reported config
and provider state.
...
### Can DeepSeek edit files outside the current repository?
