...
This document covers the security boundaries of DeepSeek Workrooms — the
...
1. **Local-first.** Future persisted workroom state should live under the
   DeepSeek home directory, protected by user-only filesystem permissions.
   No cloud sync, no telemetry, no third-party hosting.

2. **No secrets in links.** `deepseek://workroom/wr_...` URLs contain only
