...
### 1.1 `deepseek-mcp` crate (`crates/mcp/`)
...
- The `deepseek` CLI (non-interactive mode)
...
**Dependencies:** `serde`, `serde_json`, `deepseek-protocol` (for tool schema)
...
2. Re-export from `deepseek-mcp` for backward compatibility
3. Update `Cargo.toml` in `deepseek-mcp` to depend on `deepseek-mcp-protocol`
...
3. Update `deepseek-tui` to depend on `deepseek-mcp-client`
4. Update `deepseek-cli` to depend on `deepseek-mcp-server`
5. Deprecate `deepseek-mcp` crate (re-exports from new crates)
...
2. Encrypted file in `~/.deepseek/mcp-credentials/` (fallback)
...
| Breaking internal imports | Phase 2 carries `deepseek-mcp` deprecation shim for one release |
