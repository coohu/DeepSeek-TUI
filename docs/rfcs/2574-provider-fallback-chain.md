...
DeepSeek can store credentials and defaults for several providers, but a
...
Before switching to a fallback provider/model, DeepSeek checks that the
...
If no fallback candidate passes the gate, DeepSeek surfaces the original
