# `deepseek remote-setup` - Tailscale-first design
...
`deepseek remote-setup` exists today as a generate-only bundle wizard for
cloud plus chat bridge deployments, and `--apply` is still not implemented.

## Goal

Give users a guided, education-forward way to reach a local-first DeepSeek
...
> How do you want to reach DeepSeek?
...
- `deepseek app-server --http` is the canonical HTTP/SSE runtime API entrypoint.
  It delegates to the mature `serve --http` implementation.
- `deepseek app-server --mobile` is real and serves the phone control page at
  `/mobile`.
...
- `deepseek doctor --json` exists as the machine-readable local diagnostic.
- `deepseek remote-setup` exists, but today it is generate-only. Its current
...
deepseek app-server --http --host 127.0.0.1 --port 7878 --auth-token "$DEEPSEEK_RUNTIME_TOKEN"

# Runtime API plus /mobile, verified:
deepseek app-server --mobile --host 127.0.0.1 --port 7878 --auth-token "$DEEPSEEK_RUNTIME_TOKEN"
```
...
deepseek app-server --http \
  --host 127.0.0.1 \
  --port 7878 \
  --auth-token "$DEEPSEEK_RUNTIME_TOKEN"
```
...
- Exposed by DeepSeek: only the address it binds. The recommended bind is
  `127.0.0.1:7878`.
- Auth token: `DEEPSEEK_RUNTIME_TOKEN`, passed as `Authorization: Bearer ...`
...
DEEPSEEK_RUNTIME_TOKEN=<same value used to start app-server>
```
...
- Token used: `DEEPSEEK_RUNTIME_TOKEN` for control routes; local `/health` and
  `/v1/runtime/info` are public bootstrap endpoints.

### 2. Private devices with Tailscale (Recommended)

Use this to reach DeepSeek from your phone or laptop without opening a LAN or
public port. Tailscale authenticates devices in your tailnet; DeepSeek still
binds to localhost.
...
For the current binary's mobile page, start DeepSeek with the verified mobile
variant:

```bash
deepseek app-server --mobile \
  --host 127.0.0.1 \
  --port 7878 \
  --auth-token "$DEEPSEEK_RUNTIME_TOKEN"
```
...
```text
https://<machine>.<tailnet>.ts.net/mobile#token=<DEEPSEEK_RUNTIME_TOKEN>
```
...
- Not exposed: the raw DeepSeek listener; it stays on `127.0.0.1`.
- Token used: Tailscale identity gates network reachability; DeepSeek still
  uses `DEEPSEEK_RUNTIME_TOKEN` for runtime control.
...
Use this when a Telegram DM should control a local DeepSeek runtime. The bridge
...
DEEPSEEK_RUNTIME_URL=http://127.0.0.1:7878
DEEPSEEK_RUNTIME_TOKEN=<same value used to start app-server>
DEEPSEEK_WORKSPACE=/path/to/workspace
# Optional override; leave blank to inherit the runtime's configured provider/model.
DEEPSEEK_MODEL=
DEEPSEEK_MODE=agent
DEEPSEEK_ALLOW_SHELL=true     # grants shell execution from the bridge; set false for text-only chat
...
- Exposed: no inbound DeepSeek port. Telegram sees messages sent to the bot.
- Not exposed: DeepSeek remains on `127.0.0.1`; provider keys stay in the
  runtime env, not the Telegram env.
- Tokens used: `TELEGRAM_BOT_TOKEN` for Telegram, `DEEPSEEK_RUNTIME_TOKEN` for
...
Temporarily set `DEEPSEEK_ALLOW_UNLISTED=true`, message the app once, copy the
logged open id into `DEEPSEEK_CHAT_ALLOWLIST`, then set
`DEEPSEEK_ALLOW_UNLISTED=false` and restart the bridge.
...
- Exposed: no inbound DeepSeek port. Feishu/Lark sees messages sent to the app.
- Not exposed: DeepSeek remains on `127.0.0.1`; provider keys stay in runtime
  config.
- Tokens used: `FEISHU_APP_ID` / `FEISHU_APP_SECRET` for the platform,
  `DEEPSEEK_RUNTIME_TOKEN` for bridge-to-runtime calls, and
  `DEEPSEEK_CHAT_ALLOWLIST` for chat gating.
...
WEXIN_STATE_DIR=/var/lib/deepseek-weixin-bot-bridge
```
...
- Exposed: no inbound DeepSeek port. The personal Weixin session and the
...
```env
DEEPSEEK_RUNTIME_URL=https://<public-name>
DEEPSEEK_RUNTIME_TOKEN=<openssl-rand-hex-32>
...
- Not exposed by DeepSeek directly: the backend still binds to `127.0.0.1`,
  but the fronting layer makes selected routes reachable from the internet.
- Token used: `DEEPSEEK_RUNTIME_TOKEN` remains mandatory for control routes.
- Caveat: public does not mean safe. Do not use `--insecure-no-auth`, do not bind
  DeepSeek to `0.0.0.0`, and do not call this the default.
...
Use it for ideas; keep DeepSeek's style: Rust core, local runtime, zero-dep
...
- **Provider = runtime env.** The runtime resolves provider/model/API key from
  `DEEPSEEK_PROVIDER`, provider key vars, and the provider registry. Bridges do
...
- **Bridge = transport.** A chat bridge forwards allowed chat messages to
  `http://127.0.0.1:7878` with `DEEPSEEK_RUNTIME_TOKEN`.
...
- `<bridge>.env` - transport only when a bridge is selected:
  ...
  ```env
  DEEPSEEK_RUNTIME_URL=http://127.0.0.1:7878
  DEEPSEEK_RUNTIME_TOKEN=<same random token>
...
- optional `deepseek-<bridge>.service`
...
All cloud plans should bind DeepSeek to `127.0.0.1` and then layer one of the
reachability paths above.

## Namespace migration: `DEEPSEEK_*` to `DEEPSEEK_*`

Carry forward the convention already used in code: read `DEEPSEEK_X` first,
fall back to `DEEPSEEK_X` where compatibility is needed.

Touch list from the original RFC remains valid:

1. Bridges: read `DEEPSEEK_X ?? DEEPSEEK_X` for runtime URL/token, workspace,
   model, mode, shell/trust/approval flags, allowlists, and timeouts. Templates
   should emit `DEEPSEEK_*`.
2. Deploy units: prefer `/etc/deepseek/*.env`; keep legacy path reads only for
   compatibility where needed.
3. `.env.example` files and `config.example.toml`: lead with `DEEPSEEK_*`,
...
- Runtime and bridge env files share the same `DEEPSEEK_RUNTIME_TOKEN`.
- Env files lead with `DEEPSEEK_*`.
...
Verified against DeepSeek code/docs in this worktree:

- `deepseek app-server --http --host 127.0.0.1 --port 7878 --auth-token TOKEN`
- `deepseek app-server --mobile --host 127.0.0.1 --port 7878 --auth-token TOKEN`
- `deepseek doctor --json`
...
- `deepseek remote-setup --access ...` and access-path registry
