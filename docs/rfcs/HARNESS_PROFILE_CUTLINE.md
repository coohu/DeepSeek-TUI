...
For v0.9.0, DeepSeek should treat harness profiles as typed policy data first.
...
Only after those surfaces are visible and tested should DeepSeek add evidence
...
`.deepseek/constitution.json` is local repo law, not another provider profile.
...
- the repo-law source, such as `.deepseek/constitution.json` or none.
