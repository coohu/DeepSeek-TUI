# RFC: DeepSeek Workrooms — Chat-native Threaded Agent Work
...
DeepSeek agent work currently lives in transient TUI sessions, local Runtime API
...
```
deepseek://workroom/wr_abc123def456
deepseek://workroom/wr_abc123def456/thread/thr_xyz
deepseek://workroom/wr_abc123def456/event/evt_789
```
...
### 3.3 `GET /workroom/resolve?link=deepseek://workroom/wr_abc/thread/thr_x`
...
A model-visible tool that takes a `deepseek://workroom/...` URL and returns
...
- **Local-first by default.** Persisted workroom state should live under the
  DeepSeek home directory alongside existing state. No cloud service is
  assumed.
...
sidebar, and allow pasting `deepseek://` links into the composer for
context resolution.
...
- No hosted public DeepSeek cloud service
