...
1. Pull the queue (read everything, decide nothing yet):
   ```bash
   gh pr list --repo coohu/deepseek-tui --state open --limit 200 \
   ...
   gh issue list --repo coohu/deepseek-tui --state open --limit 300 \
...
3. Confirm each shortlisted PR from code, tests, comments, and checks:
   ...
   gh pr checks N --repo coohu/deepseek-tui
   ```
