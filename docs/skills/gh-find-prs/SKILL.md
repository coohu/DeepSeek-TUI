---
name: gh-find-prs
description: "Survey open DeepSeek PRs and triage each for mergeability and disposition against the real landing branch."
---
...
3. **Read each candidate from code, not title.** For every non-trivial PR:
   ```
   gh pr view <N> --repo coohu/deepseek-tui \
     --json files,additions,deletions,statusCheckRollup,body,comments
   gh pr diff <N> --repo coohu/deepseek-tui
   ```
...
4. **Decode check failures — distinguish trivial from real.** In `statusCheckRollup`, find each `conclusion: FAILURE` and read its job. DeepSeek's CI jobs are `Lint`, `Test (ubuntu-latest|macos-latest|windows-latest)`, `Version drift`, `gate` (Contribution gate), `npm wrapper smoke`, `Mobile runtime smoke`, `Documentation`, `GitGuardian Security Checks`.
