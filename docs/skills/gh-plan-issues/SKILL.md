...
- Repo root: the local DeepSeek checkout (run `git rev-parse --show-toplevel`).
- GitHub repo: `coohu/deepseek-tui`
- GitHub CLI: `gh`
...
```bash
gh issue list --repo coohu/deepseek-tui --milestone v0.8.62 \
...
The runtime control plane leads; UI/docs ride along. For DeepSeek the lead
train, in order, is:
