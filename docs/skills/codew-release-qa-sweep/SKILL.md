...
description: "Use before claiming DeepSeek release work is done: run the full gate sweep and list the manual QA targets."
---

# DeepSeek Release QA Sweep

Run this before claiming any DeepSeek release work is "done." A green automated
...
cargo test -p deepseek-config -p deepseek-protocol -p deepseek-cli \
  -p deepseek-whaleflow -p deepseek-state --locked

# 3. TUI test binaries, locked
cargo test -p deepseek-tui --bins --locked

# 4. TUI debug build, locked
cargo build -p deepseek-tui --locked
