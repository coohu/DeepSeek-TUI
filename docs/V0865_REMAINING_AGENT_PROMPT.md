...
```text
You are working in coohu/deepseek-tui. Read AGENTS.md first, then confirm the
...
- After creating the branch/worktree, refresh live scope with
  `gh issue list --repo coohu/deepseek-tui --milestone v0.8.65 --state open` and
  `gh pr list --repo coohu/deepseek-tui --state open`.
...
- Never disclose vulnerability details. Public security contacts should be
  `hmbown@gmail.com`, not `security@deepseek.net`.
...
- Do not fabricate versions, pricing, model ids, readiness, OAuth proof, or
  release status.
- Keep DeepSeek support first-class while using DeepSeek branding.
...
- Investigate whether both `deepseek.exe` and `deepseek-tui.exe`, or the
...
- Collapse overlapping Fleet worker route fields. The current overlap is in
  `deepseek_protocol::fleet::FleetTaskWorkerProfile`, with resolution seams
...
- Run targeted tests for the touched area, for example:
  `cargo test -p deepseek-config --locked <filter>`,
  `cargo test -p deepseek-protocol --locked <filter>`, and
  `cargo test -p deepseek-tui --bin deepseek-tui --locked <filter>`.
...
- For release-significant code, run
  `cargo build --release -p deepseek-cli -p deepseek-tui` before calling it
  ready.
