# `deepseek remote-setup` — Design & Implementation Plan
...
One command — `deepseek remote-setup` — that guides a user through standing up
a remote DeepSeek agent they can talk to from a phone chat app, across:
...
same problem and **validates this design**. Use it for ideas; keep DeepSeek's
...
```rust
/// Provision and configure a remote DeepSeek agent (cloud + chat bridge).
...
| `--out <dir>` | Bundle output dir (default `./deepseek-deploy/<cloud>-<bridge>`). |
...
pub struct BridgeSpec {
    ...
    pub service_unit: &'static str,    // "deepseek-telegram-bridge.service"
...
Written to `./deepseek-deploy/<cloud>-<bridge>/`:
...
- `deepseek-runtime.service`, `deepseek-<bridge>.service`.
...
3. `az vm create` with `--custom-data cloud-init.yaml` and a **system-assigned
   managed identity**; cloud-init pulls `ghcr.io/hmbown/deepseek:latest`, reads
   the secrets from Key Vault via the identity, writes `/etc/deepseek/*.env`,
...
2. **Deploy units** (`deploy/tencent-lighthouse/systemd/*`,
   ...
   env file paths `/etc/deepseek/` → `/etc/deepseek/` (keep reading the old path
   if present).
