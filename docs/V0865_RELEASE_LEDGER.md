...
reports npm plus all `deepseek-*` crates unpublished. `origin/main` has moved
...
| Area | Evidence |
| --- | --- |
| Public security contact | #3558 updates `SECURITY.md`, release checks, and web footer to use `hmbown@gmail.com` after `security@deepseek.net` bounced. No vulnerability details are recorded here. |
...
- #3560: `cargo fmt --all --check`; `cargo test -p deepseek-config --locked harness`;
  `cargo test -p deepseek-config --locked`; `git diff --check`.
