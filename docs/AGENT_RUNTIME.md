# The DeepSeek Agent Runtime — one durable substrate, familiar launchers
...
- **`deepseek exec`** is the headless front door: usable by anyone at any time
...
- A **fleet worker** *is* a `deepseek exec` run that the fleet launches and
  tracks durably — locally as a subprocess, or remotely as
  `ssh host … deepseek exec …`. The fleet does not re-implement execution; it
...
then the cutover is incomplete. Treat that as a DeepSeek runtime gap, not as
...
`deepseek exec`. The heavy construction then lives in another process entirely,
...
simulated taxonomy. `deepseek exec --output-format stream-json` emits
...
DeepSeek should converge with Claude Code on **shape**, not on branding:
...
- **Keep distinct**: DeepSeek branding and first-class DeepSeek/GLM/MiniMax/
  multi-provider support; the local-first **Agent Fleet** (durable, SSH-capable
  orchestration) as DeepSeek's own layer above the shared runtime; WhaleFlow as
