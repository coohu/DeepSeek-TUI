...
## 1. The architectural spine: WhaleFlow = ultracode, for DeepSeek's many models
...
what **WhaleFlow** should be *inside* DeepSeek. The DeepSeek-specific twist, and the reason it is
not just a Claude-Code clone: **DeepSeek's workers are heterogeneous model types.** A flash model
...
4. **Methodical merge**: cherry-pick/merge green branches into `codex/v0.8.61` one at a time,
   re-running the gate (`cargo test -p deepseek-tui --bins`, fmt, `git diff --check`) after each.
