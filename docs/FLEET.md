...
`deepseek exec` run that the fleet launches and tracks durably. See
...
```sh
deepseek fleet init
deepseek fleet run tasks.json --max-workers 4
deepseek fleet status
deepseek fleet inspect <worker-id>
deepseek fleet logs <worker-id>
deepseek fleet artifacts <worker-id>
deepseek fleet interrupt <worker-id>
deepseek fleet restart <worker-id>
deepseek fleet resume <run-id>
deepseek fleet stop --all
```

`deepseek fleet resume <run-id>` is the restart-recovery verb: it replays the
...
Fleet state is stored under the workspace in `.deepseek/fleet.jsonl`. Worker
logs and adapter logs are stored under `.deepseek/fleet/` and
`.deepseek/fleet-host/`.
...
perform real work as `deepseek exec` processes.
...
`deepseek fleet run` accepts JSON or TOML. A minimal JSON spec:
...
Workers are optional. If omitted, DeepSeek creates local worker slots up to
`--max-workers`.
...
Workers write bounded artifact files under `.deepseek/fleet/` and ledger only
...
the source as `transport`, `task`, or `verifier`. `deepseek fleet status`
...
```json
{
  ...
  "scorer": { "kind": "regex_match", "path": ".deepseek/fleet/report.md", "pattern": "finding|all clear" }
}
```
...
```json
{
  ...
  "adapters": {
    "ops-slack": {
      "kind": "slack",
      "webhook_env": "DEEPSEEK_FLEET_SLACK_WEBHOOK",
      "channel": "#deepseek-fleet"
    },
    "pager": {
      "kind": "pager_duty",
      "routing_key_env": "DEEPSEEK_FLEET_PAGERDUTY_ROUTING_KEY",
      "severity": "critical"
    }
  }
}
...
safe inspection commands such as `deepseek fleet status` and
`deepseek fleet inspect <worker-id>`. Endpoints, webhook secrets, and
PagerDuty routing keys are shown as `<redacted:env:...>`.

## Status Surfaces

`deepseek fleet status` shows compact counts for queued, running, completed,
...
work. Start with `deepseek fleet status`, then inspect one run or worker with
`deepseek fleet inspect <worker-id>`, `logs`, and `artifacts`. Use direct
reads of `.deepseek/fleet.jsonl`, host logs, or remote files only when the
...
- Restart a worker only when the failure is transient, retry budget remains,
  ...
  involved: `deepseek fleet restart <worker-id>`.
- Interrupt or stop only when the current task is unsafe to continue or the
  operator explicitly asks for cancellation: `deepseek fleet interrupt
  <worker-id>` or `deepseek fleet stop --all`.
...
```text
DeepSeek fleet needs attention
Run: <run-id>
Worker: <worker-id>
...
Latest typed evidence: deepseek fleet inspect <worker-id>; deepseek fleet artifacts <worker-id>
...
```json
{
  ...
  "host": {
    ...
    "user": "deepseek",
    "port": 22,
    "identity": "~/.ssh/deepseek_fleet",
    "working_directory": "/srv/deepseek/work",
    "env_allowlist": ["DEEPSEEK_PROFILE"],
    "deepseek_binary": "/usr/local/bin/deepseek"
  },
...
- SSH requires an explicit host, working directory, and DeepSeek binary path;
- secret-like environment names such as `TOKEN`, `SECRET`, `PASSWORD`,
  `API_KEY`, and `PRIVATE_KEY` are rejected from adapter allowlists;
- secrets should remain in DeepSeek config providers or remote host config,
...
|-------|--------|----------|
| `sandbox` | No network, no secrets, writes only to `.deepseek/fleet/` | Nothing — default for new workers |
...
```json
{
  "security_policy": {
    ...
    "allowed_secrets": [
      ...
      {"key": "DEEPSEEK_API_KEY", "source": "keyring"}
    ],
    "capability_grants": [
      {
        "capability": "network",
...
- `"file"` — resolve from `~/.deepseek/secrets/`
...
```json
{
  "kind": "slack",
  "webhook": {
    "url_ref": {"key": "DEEPSEEK_FLEET_SLACK_WEBHOOK", "source": "env"},
    "secret_ref": {"key": "DEEPSEEK_FLEET_SLACK_SIGNING_SECRET", "source": "keyring"}
  }
}
```
