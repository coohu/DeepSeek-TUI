...
Workrooms are DeepSeek's chat-native abstraction for durable, addressable
...
1. **Creation.** A future workroom is created when a thread is started with a
   ...
   is stable and can be shared as a `deepseek://workroom/...` link.
...
3. **Link resolution.** When a `deepseek://workroom/...` link appears in
...
Persisted workroom state should live alongside existing DeepSeek state:
...
|---|---|
| `deepseek-protocol` | Types: `Workroom`, `WorkroomId`, `WorkroomThread`, `WorkroomEvent`, `WorkroomLink`, `ExternalThreadRef`, `AgentAttribution` |
| `deepseek-app-server` | Future endpoints: `GET /workrooms`, `GET /workroom/:id/threads`, `GET /workroom/resolve` |
| `deepseek-tui` | Future model-facing link resolution and optional sidebar inbox |
| `deepseek-state` | Future: persistent workroom store (Phase 2) |
