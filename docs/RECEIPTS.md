...
runtime records that DeepSeek already owns: thread metadata, turn status, turn
...
```text
deepseek receipt export --thread <thread_id> --turn <turn_id> --format json
...
`deepseek review --write-receipt` writes a local JSON receipt for the reviewed
diff under the DeepSeek state directory (`review-receipts/`) unless
...
`deepseek review --write-receipt` after changing the diff; reviewers should
compare the `diff_fingerprint` before reusing a receipt in a PR handoff.

`deepseek review --check-receipt` is the local pre-push gate. It does not call
...
```json
{
  "schema_id": "deepseek.conformance-receipt/v0",
  "thread": {
    "id": "thr_...",
