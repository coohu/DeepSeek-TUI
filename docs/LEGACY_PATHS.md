...
DeepSeek was renamed from DeepSeek-TUI. To avoid breaking existing installs, the runtime reads
state from the new `~/.deepseek/` location but **falls back** to the legacy `~/.deepseek/` location,
and always **writes** to `~/.deepseek/`. This doc audits each legacy reference and records a
...
| Symbol | Line | Purpose |
|---|---|---|
| `DEEPSEEK_APP_DIR = ".deepseek"` | 3428 | canonical app dir |
| `LEGACY_APP_DIR = ".deepseek"` | 3431 | legacy app dir (fallback only) |
| `deepseek_home()` | 3437 | `~/.deepseek` |
| `legacy_deepseek_home()` | 3451 | `~/.deepseek` (legacy) |
| `resolve_state_dir(subdir)` | 3469 | **read** path: `~/.deepseek/<subdir>`, falling back to `~/.deepseek/<subdir>` when only the legacy dir exists |
| `ensure_state_dir(subdir)` | 3484 | **write** path: always creates under `~/.deepseek/<subdir>` |

Migration contract: read-with-fallback, write-to-new. This preserves the v0.8.44 migration for
users who still have `~/.deepseek/` while steering all new writes to `~/.deepseek/`.
...
that actively migrates `~/.deepseek/` → `~/.deepseek/` on first run and a deprecation window.
...
`resolve_state_dir`/`ensure_state_dir` instead of joining `.deepseek`/`.deepseek` by hand — is a
