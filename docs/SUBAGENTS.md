...
retry budget is exhausted, DeepSeek preserves a checkpoint and returns a
...
For parallel edit lanes, launch the child with `worktree: true`. DeepSeek
...
`.deepseek-worktrees/`, so the parent checkout stays clean.
...
- `worktree_path`: exact checkout path. Relative paths stay under the default
  sibling `.deepseek-worktrees/` root.
...
SCOPE: cargo test -p deepseek-tui --bin deepseek-tui --locked prompt; subagent filter if needed.
...
`[subagents].max_concurrent` in `~/.deepseek/config.toml`; the default equals
...
`agent`, can extend the timeout in `~/.deepseek/config.toml`:
...
`.deepseek/state/subagents.v1.json`. The record is the current run-ledger
