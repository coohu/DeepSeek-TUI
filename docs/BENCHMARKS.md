...
DeepSeek integrates with three external benchmarks to measure real-world
coding-agent performance. Each benchmark tests a different surface:

| Benchmark | What it tests | Harness | Output format |
|---|---|---|---|
| **SWE-bench** | Patch generation from GitHub issues | DeepSeek built-in (`deepseek swebench`) | `all_preds.jsonl` |
...
DeepSeek has built-in SWE-bench support via `deepseek swebench run` and
`deepseek swebench export`. See [docs/SWEBENCH.md](SWEBENCH.md) for the
...
DeepSeek plugs in via a Harbor adapter (`scripts/benchmarks/harbor/deepseek_agent.py`).
