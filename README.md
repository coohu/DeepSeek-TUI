...
DeepSeek is a terminal coding agent — a TUI and a CLI. You point it at a model
...
pick a provider and a model; DeepSeek resolves a real route and runs.
...
one model. Multi-provider support followed, and the project became DeepSeek to
match. If there's a model, endpoint, or feature you don't see that you want,
open an issue — that's how the project grows.

[简体中文 README](README.zh-CN.md) · [日本語 README](README.ja-JP.md) · [Tiếng Việt README](README.vi.md) · [한국어 README](README.ko-KR.md) · [deepseek.net](https://deepseek.net/) · [Install guide](docs/INSTALL.md) · [Provider registry](docs/PROVIDERS.md) · [Changelog](CHANGELOG.md)
...
![DeepSeek running in a terminal](assets/screenshot.png)
...
Releases and installs `deepseek`, `codew`, and `deepseek-tui`. Prefer building
...
# Windows
scoop install deepseek        # or the NSIS installer from GitHub Releases
...
to [GitHub Releases](https://github.com/coohu/deepseek-tui/releases). Checksums,
...
then run `deepseek doctor` to confirm.
...
`deepseek auth set --provider anthropic` — or just export
`ANTHROPIC_API_KEY` — and the native Messages adapter takes it from there.

Keys land in `~/.deepseek/config.toml`; legacy `~/.deepseek/` config is still
...
- `/skills` loads reusable workflows from `~/.deepseek/skills/`.
- `/config` edits runtime settings; `/statusline` chooses which footer chips
  show route, cost, and session state.
- `! cargo test -p deepseek-tui` runs any shell command through the normal
...
You pick a provider and a model, and DeepSeek resolves a **real route** — a
...
- **Honest cost display.** A route reports exactly one cost state: per-token
  ...
  applicable*, or *unknown / stale*. DeepSeek never invents a price it doesn't
...
Fleet is DeepSeek's durable control plane for multi-worker runs. A fleet worker
is a headless `deepseek exec` run, but the fleet launches and tracks it durably:
work is recorded in an append-only ledger (`.deepseek/fleet.jsonl`), so a run
...
DeepSeek edits files and runs commands, so the safety posture is part of the
...
- **Approval-gated tools.** A `.deepseek/hooks.toml` hook system can allow,
...
- **Headless mode.** `deepseek exec` with `--allowed-tools`,
...
- **MCP, bidirectionally.** Consume tools from external MCP servers, or expose
  DeepSeek itself as an MCP server via `deepseek mcp`.
- **Skills.** Reusable workflows in `~/.deepseek/skills/`, loaded with
  `/skills`.
...
guess. DeepSeek uses a **nested constitution** so there's a defined rank instead
of vibes.
...
2. **Your project's law** — drop a `.deepseek/constitution.json` in a repo to
...
[deepseek.net](https://deepseek.net/):
...
DeepSeek started as one person's DeepSeek side project. Developers from
...
- [Open issues](https://github.com/coohu/deepseek-tui/issues) — good first
...
- [Contributors](docs/CONTRIBUTORS.md) — the people who've shaped DeepSeek.

Support: [Buy me a coffee](https://www.buymeacoffee.com/hmbown).

## Thanks

DeepSeek exists because of the people who use it, break it, and fix it.
...
> *DeepSeek is an independent community project and is not affiliated with any
> model provider.*
