...
function resolveRepo() {
  return process.env.DEEPSEEK_TUI_GITHUB_REPO || process.env.DEEPSEEK_GITHUB_REPO || "coohu/DeepSeek-TUI";
}
...
function hasReleaseBaseOverride() {
  return Boolean(
    process.env.DEEPSEEK_RELEASE_BASE_URL ||
      ...
      process.env.DEEPSEEK_RELEASE_BASE_URL ||
      process.env.DEEPSEEK_USE_CNB_MIRROR,
  );
}
...
function assertPackageVersionMatchesBinaryVersion(version) {
  ...
  if (process.env.DEEPSEEK_ALLOW_NPM_BINARY_MISMATCH === "1") {
    console.log(
  ...
  throw new Error(
    `npm package version ${pkg.version} does not match codewhaleBinaryVersion ${version}. ` +
      "Set DEEPSEEK_ALLOW_NPM_BINARY_MISMATCH=1 only for an intentional packaging-only npm release.",
  );
}
